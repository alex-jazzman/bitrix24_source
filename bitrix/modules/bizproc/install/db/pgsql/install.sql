...
CREATE INDEX ix_b_bp_workflow_instance_modified ON b_bp_workflow_instance (modified);
...
CREATE INDEX ix_bp_task_archive_tasks_competed_at_archive ON b_bp_task_archive_tasks(COMPLETED_AT, ARCHIVE_ID);
...
CREATE UNIQUE INDEX ux_bp_wts_template_trigger ON b_bp_workflow_trigger_schedule (template_id, trigger_name);
CREATE INDEX ix_bp_wts_next ON b_bp_workflow_trigger_schedule (next_run_at);
...
CREATE INDEX ix_b_bp_mwrm_queue_id_available_at ON b_bp_messenger_workflow_resume_message (QUEUE_ID, AVAILABLE_AT);

CREATE TABLE IF NOT EXISTS b_bp_debug
(
	ID int8 GENERATED BY DEFAULT AS IDENTITY NOT NULL,
	USER_ID int NOT NULL,
	TEMPLATE_ID int NOT NULL,
...
CREATE INDEX IX_BP_DEBUG_USER_ID ON b_bp_debug (USER_ID);
CREATE INDEX IX_BP_DEBUG_USER_TEMPLATE_ID ON b_bp_debug (USER_ID, TEMPLATE_ID);

CREATE TABLE b_bp_debug_session
(
	ID int8 GENERATED BY DEFAULT AS IDENTITY NOT NULL,
	DEBUG_ID int8 NOT NULL,
	USER_ID int NOT NULL,
	MODULE_ID varchar(32) NULL,
	ENTITY varchar(64) NULL,
	DOCUMENT_ID varchar(128) NULL,
...
(
	ID int8 GENERATED BY DEFAULT AS IDENTITY NOT NULL,
	DEBUG_SESSION_ID int8 NOT NULL,
...
CREATE INDEX IX_BP_DEBUG_SESSION_ID ON b_bp_debug_trace (DEBUG_SESSION_ID);
...
CREATE TABLE b_bp_storage_record_field (
	...
	VALUE text NOT NULL,
	VALUE_NUM decimal DEFAULT NULL,
	PRIMARY KEY (ID)
);
CREATE INDEX ix_storage_record_field_record_field ON b_bp_storage_record_field (RECORD_ID, FIELD_ID);
CREATE INDEX ix_storage_record_field_value_record ON b_bp_storage_record_field (FIELD_ID, VALUE, RECORD_ID);
CREATE INDEX ix_storage_record_field_num_record ON b_bp_storage_record_field (FIELD_ID, VALUE_NUM, RECORD_ID);
