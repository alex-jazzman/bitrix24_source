...
{
	ShowError('bitrix.xdebug module is not installed');
	return;
}
...
if ($_REQUEST['mode'] == 'src')
{
	...
	?><html><body>
		...
		if ($XTR->Start($trace_output_dir.'/'.$trace, $pos))
		{
			...
			<script>
				...
				window.addEventListener("DOMContentLoaded", function() {HighlightLine()});
			</script>
			<?
			exit();
		}
	...
	if ($line)
		echo '<script>window.addEventListener("DOMContentLoaded", function() {HighlightLine('.$line.')});</script>';
	exit();
}
...
<iframe id="src_iframe" style="border:1px solid #ccc;width:100%;" src=""></iframe>
<?
