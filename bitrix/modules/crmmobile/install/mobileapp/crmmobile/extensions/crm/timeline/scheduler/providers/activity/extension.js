	...
	class TimelineSchedulerActivityProvider extends TimelineSchedulerBaseProvider
	{
			...
			return Toolbar({
				...
				center: () => View(
					...
					ToolbarIcon({
						...
						height: 19,
						onClick: () => {
							if (ATTACH_ALLOWED && this.fileFieldRef)
							{
		...
		save()
		{
			return new Promise((resolve, reject) => {
				...
				const { text, deadline, files } = this.state;
				...
				const data = {
					...
					files: files.map(file => file.token),
