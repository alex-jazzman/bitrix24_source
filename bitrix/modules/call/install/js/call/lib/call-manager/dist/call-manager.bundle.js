...
(function (exports,main_core_events,ui_vue3_vuex,call_core,im_public,im_v2_lib_slider,im_v2_const,im_v2_lib_logger,im_v2_lib_promo,im_v2_lib_soundNotification,im_v2_lib_desktopApi,rest_client,call_lib_callSliderManager,im_call_compatible,main_core,ui_buttons,im_v2_application_core) {
	...
	class CallManager {
	  ...
	  startCall(dialogId, withVideo = true, callToken = '') {
	    ...
	    call_lib_callSliderManager.CallSliderManager.getInstance().setTopSliderId();
	  ...
	  leaveCurrentCall() {
	    ...
	    call_lib_callSliderManager.CallSliderManager.getInstance().clearSliderId();
	  }
	  ...
	  deleteRecentCall(dialogId) {
	    im_v2_application_core.Core.getStore().dispatch('recent/calls/deleteActiveCall', {
	      dialogId
	    });
	  }
	  foldCurrentCall() {
	  ...
	  hasActiveCurrentCall(dialogId) {
	    return im_v2_application_core.Core.getStore().getters['recent/calls/hasActiveCall'](dialogId) && this.getCurrentCallDialogId() === dialogId;
	  }
	...
	function _onCallCreated2(event) {
	  ...
	  if (isNewCall) {
	    ...
	    im_v2_application_core.Core.getStore().dispatch('recent/calls/addActiveCall', {
	  ...
	  im_v2_application_core.Core.getStore().dispatch('recent/calls/updateActiveCall', {
	    dialogId: call.associatedEntity.id,
	    fields: {
	      name: call.associatedEntity.name,
	      state,
