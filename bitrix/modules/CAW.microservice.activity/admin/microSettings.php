...
if ($request["apply"] == "Применить") {
    $token = $request["token"];
    $url = $request["url"];
    COption::SetOptionString("main", "mws_microserv_token", serialize($token));
    COption::SetOptionString("main", "mws_microserv_url", serialize($url));
} else {
    $token = unserialize(COption::GetOptionString("main", "mws_microserv_token", "a:0:{}"));
    $url = unserialize(COption::GetOptionString("main", "mws_microserv_url", "a:0:{}"));
}
