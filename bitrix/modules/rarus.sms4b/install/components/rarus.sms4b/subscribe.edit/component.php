...
if (preg_match("/^[ \+\-\(\)0-9]+?$/i", $_REQUEST['sf_EMAIL'])) {
    $sms4b = new Csms4b();
    $_REQUEST['sf_EMAIL'] = $sms4b->normalizePhoneNumber($_REQUEST['sf_EMAIL']);
...
if ($USER->IsAuthorized()) {
    ...
    if ($to == '') {
        ...
        if ($to == '' && preg_match("/^[ \+\-\(\)0-9]+$/i",
                $_REQUEST['sf_EMAIL']) && $arResult['SHOW_SMS_FORM'] && $arUser['PERSONAL_MOBILE'] == '') {
            $user = new CUser;
            $fields = [
...
if ($to == '' && $_REQUEST['sf_EMAIL'] <> '' && (($is_sms && $arResult['SHOW_SMS_FORM'])
        || ($arResult['SHOW_POST_FORM'] && !$is_sms))) {
...
//pass
if ($_REQUEST['CONFIRM_CODE'] == '' && $_SESSION['AUTH_PASS_EMAIL'] && $arResult['SHOW_POST_FORM']
    && $_REQUEST['AUTH_PASS'] == '') {
    $_REQUEST['AUTH_PASS'] = $_SESSION['AUTH_PASS_EMAIL'];
} elseif ($_REQUEST['CONFIRM_CODE'] == '' && $_SESSION['AUTH_PASS_SMS'] && $arResult['SHOW_SMS_FORM']
    && $_REQUEST['AUTH_PASS'] == '') {
...
if ($arResult['SHOW_POST_FORM']) {
    $cdb_sub_check = CUser::GetList(($by = 'ID'), ($order = 'desc'), ['EMAIL' => $to]);
    $user_inf = $cdb_sub_check->Fetch();
} elseif ($arResult['SHOW_SMS_FORM']) {
    $cdb_sub_check = CUser::GetList(($by = 'ID'), ($order = 'desc'),
        ['PERSONAL_MOBILE' => kill_post_fix($to)]);
    $user_inf = $cdb_sub_check->Fetch();
}
...
if ($ACTIVE_FORM) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_REQUEST['PostAction']) && check_bitrix_sessid()) {
        ...
        if (!empty($_REQUEST['LOGIN'])) {
            ...
            //new user
            $res = $USER->Register($_REQUEST['NEW_LOGIN'], '', '', $_REQUEST['NEW_PASSWORD'],
                $_REQUEST['CONFIRM_PASSWORD'], $_REQUEST['EMAIL'], false, $_REQUEST['captcha_word'],
...
if ($ID > 0 && !$obSubscription->IsAuthorized($ID)) {
    ...
    if (!$obSubscription->Authorize($ID,
            (empty($_REQUEST['AUTH_PASS']) ? $_REQUEST['CONFIRM_CODE'] : $_REQUEST['AUTH_PASS']))
        && !empty($_REQUEST['AUTH_PASS'])) {
        $arWarning[] = GetMessage('error_in_pass');
    } else {
        $obSubscription->Update($ID, ['CONFIRM_CODE' => $_REQUEST['AUTH_PASS']]);
    }
}

if ($obSubscription->IsAuthorized($ID)) {
    if ($arResult['SHOW_SMS_FORM'] && preg_match("/^[ \+\-\(\)0-9]+?@phone.sms$/",
            $to) && $_REQUEST['auth_type'] == 'sms') {
...
if ($ACTIVE_FORM) {
    if ($ID > 0
        && (($_REQUEST['action'] == 'unsubscribe') || ($_REQUEST['action'] == 'activate') || check_bitrix_sessid())) {
...
foreach ($arRubrics as $arRubric) {
    ...
    if (!str_contains($arRubric['NAME'], 'SMS') && !str_contains($arRubric['DESCRIPTION'], 'SMS')) {
        $ARN = 'EMAIL';
    } else {
        $ARN = 'SMS';
    }
...
if ($arSubscr && $to <> '' && (($is_sms && $arResult['SHOW_SMS_FORM']) || (!$is_sms && $arResult['SHOW_POST_FORM']))) {
    ...
    $arResult['REQUEST']['LOGIN'] = htmlspecialchars(($_REQUEST['LOGIN'] ?? $sLastLogin));
