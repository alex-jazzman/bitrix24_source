...
{
    $subscription_ureg = CSubscription::GetList(['ID' => 'ASC'], ['USER_ID' => $userid]);
    while (($subscr_arr = $subscription_ureg->Fetch())) {
        ...
        } elseif ($arResult['SHOW_POST_FORM'] && !preg_match("/^[ \+\-\(\)0-9]+?@phone\.sms$/i",
                $subscr_arr['EMAIL'])) {
            return $subscr_arr['EMAIL'];
        }
    }
...
class  CSubscriptionMod extends CSubscription
{
    ...
    public static function ConfirmEvent($ID, $SITE_ID = SITE_ID)
    {
        ...
        if ($subscr_arr = $subscr->Fetch()) {
            ...
            CEvent::Send('SUBSCRIBE_CONFIRM', $SITE_ID, $arFields, 'Y');
            return true;
        }
        return false;
    }
