...
?>
<?php if ($arResult['CURRENT_USER']['DEPARTMENT_TOP']):?>
...
<input type="hidden" name="current_filter" value="adv" />
    ...
    <?php if ($arResult['FILTER_VALUES'][$arParams['FILTER_NAME'].'_LAST_NAME']):?>
...
<tr>
	...
	<td>
        <?php if ($arResult['CURRENT_USER']['DEPARTMENT_TOP']):?>
		<input type="checkbox" id="only_mine_office" onclick="BXChangeFilterTop_adv(this)" <?=$arResult['FILTER_VALUES'][$arParams['FILTER_NAME'].'_UF_DEPARTMENT'] == $arResult['CURRENT_USER']['DEPARTMENT_TOP'] || $arResult['FILTER_VALUES'][$arParams['FILTER_NAME'].'_UF_DEPARTMENT'] == [$arResult['CURRENT_USER']['DEPARTMENT_TOP']] ? 'checked="checked"' : ''?> />
