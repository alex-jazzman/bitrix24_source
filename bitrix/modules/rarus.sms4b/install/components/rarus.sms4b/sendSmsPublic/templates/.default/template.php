...
<div id="officer">
                ...
                <? else: ?>
                    <? foreach ($arResult['ADDRESSES'] as $arIndex): ?>
                        <option value="<?= $arIndex ?>"
                                <? if ($arIndex == htmlspecialchars($arResult['ADDRESSES_BY_DEFAULT'])): ?>selected<? endif; ?>><?= $arIndex ?></option>
                    <? endforeach; ?>
                <? endif; ?>
            </select>
        </div>
	...
	false
		...
		<div id="messageText">
            ...
            <div>
                <b><label for="TRANSLIT"><?=GetMessage('SMS4B_PUBLIC_COMPONENT_TRANSLIT_TO')?></label></b>
                <input type="checkbox" id="TRANSLIT" name="TRANSLIT" value="Y"
                       <?php if ($_REQUEST['TRANSLIT']): ?>checked<? endif; ?>/>
                <span class="comments"><?=GetMessage('SMS4B_PUBLIC_COMPONENT_TRANSLIT_TO_DESCRIPTION')?></span>
            </div>
