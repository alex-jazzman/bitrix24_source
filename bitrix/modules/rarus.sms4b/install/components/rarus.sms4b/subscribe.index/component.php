...
if ($USER->IsAuthorized()) {
    $ar_user_subs = CSubscription::GetList(['ID' => 'asc'], ['USER_ID' => $USER->GetID()]);
    $post_sub = '';
    $sms_sub = '';
...
//if there is no subscribtion, we fill it with emails and telephones
    if (isset($post_sub['EMAIL']) && $post_sub['EMAIL'] == '') {
        $post_sub['EMAIL'] = $USER->GetEmail();
    }

    if (isset($sms_sub['EMAIL']) && $sms_sub['EMAIL'] == '') {

        $rsU = CUser::GetList($by = 'ID', $sort = 'desc', ['ID' => $USER->GetID()]);
        if ($arU = $rsU->Fetch()) {
    ...
    $arResult['RUBRICS_POST'] = [];
    if (isset($post_sub['ID'])) {
        $checked_rub_post = CSubscription::GetRubricList($post_sub['ID']);
    }

    if(isset($checked_rub_post)) {
        while ($rub = $checked_rub_post->Fetch()) {
            $arResult['RUBRICS_POST'][] = $rub['ID'];
        }
    }
    //get rubrics for sms
    $arResult['RUBRICS_SMS'] = [];
    if (isset($sms_sub['ID'])) {
        $checked_rub_sms = CSubscription::GetRubricList($sms_sub['ID']);
    }
    if (isset($checked_rub_sms)) {
        while ($rub_sms = $checked_rub_sms->Fetch()) {
            $arResult['RUBRICS_SMS'][] = $rub_sms['ID'];
        }
    }
    //what label on button 'll be
    if(isset($arResult['SMS_SUB']['ID'])){
        if ($arResult['SMS_SUB']['ID'] > 0 && $USER->IsAuthorized()
            || CSubscription::IsAuthorized($arResult['SMS_SUB']['ID'])) {
            $arResult['SUBMIT_SMS_BUTTON_NAME'] = 'UPD';
        }
    }
    if(isset($arResult['POST_SUB']['ID'])) {
        if ($arResult['POST_SUB']['ID'] > 0 && $USER->IsAuthorized()
            || CSubscription::IsAuthorized($arResult['POST_SUB']['ID'])) {
...
$arResult['ALL_RUBRICS'] = [];
$arResult['ALL_RUBRICS']['EMAIL'] = [];
$arResult['ALL_RUBRICS']['SMS'] = [];
