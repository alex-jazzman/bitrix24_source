...
$arParams['FILTER_NAME'] = 
		(strlen($arParams['FILTER_NAME'])<=0 || !preg_match('^[A-Za-z_][A-Za-z01-9_]*$', $arParams['FILTER_NAME'])) ?
...
if ($GLOBALS[$arParams['FILTER_NAME'].'_UF_DEPARTMENT'])
	$arFilter['UF_DEPARTMENT'] = 
		...
		: [$GLOBALS[$arParams['FILTER_NAME'].'_UF_DEPARTMENT']];
...
while ($arUser = $dbUsers->Fetch())
{
	...
	if (is_array($arUser['UF_DEPARTMENT']) && count($arUser['UF_DEPARTMENT']) > 0)
	{
		...
		if (count($arNewDep) > 0)
		{
			$dbRes = CIBlockSection::GetList(['SORT' => 'ASC'], ['ID' => $arNewDep]);
