...
global $APPLICATION, $USER, $component;
...
foreach ($arResult['EVENT'] as $arEvent)
{
	...
	{
		$arEvent['FILE_HTML'] = '<div class="event-detail-files"><label class="event-detail-files-title">'
			. GetMessage('CRM_EVENT_TABLE_FILES') . ':</label><div class="event-detail-files-list">'
		;

		foreach($arEvent['FILES'] as $key => $value)
		{
			$arEvent['FILE_HTML'] .= '<div class="event-detail-file"><span class="event-detail-file-number">'
				. $key . ' . </span><span class="event-detail-file-info"><a href="' . htmlspecialcharsbx($value['PATH'])
				. '" target="_blank" class="event-detail-file-link">' . htmlspecialcharsbx($value['NAME'])
				. '</a><span class="event-detail-file-size">(' . htmlspecialcharsbx($value['SIZE']) . ')</span></span></div>'
			;
		}
	...
	$authorHtml = '';
	if (isset($arEvent['CREATED_BY_FULL_NAME']) && $arEvent['CREATED_BY_FULL_NAME'] !== '')
	{
		$photoUrl = $arEvent['CREATED_BY_PHOTO_URL'] ?? '';
	...
	{
		$entityTypeId = CCrmOwnerType::ResolveID($arEvent['ENTITY_TYPE']);
		$description = CCrmOwnerType::GetDescription($entityTypeId);
		...
		$arColumns['ENTITY_TITLE'] = !empty($arEvent['ENTITY_TITLE'])
			? (new MiniCardHtmlBuilder($entityTypeId, (int)$arEvent['ENTITY_ID']))
	...
	{
		unset($arEvent['ENTITY_TYPE'], $arEvent['ENTITY_TITLE']);
	}
	...
	$arResult['GRID_DATA'][] = [
		...
		'data' => $arEvent,
		'actions' => $arActions,
		'editable' => $USER->IsAdmin() || ($arEvent['CREATED_BY_ID'] == $USER->GetId() && $arEvent['EVENT_TYPE'] == 0),
		'columns' => $arColumns,
	];
}
...
)
{
	...
	if (
		isset($arResult['ENTITY_TYPE'], $arResult['ENTITY_ID'])
		&& $arResult['ENTITY_TYPE'] !== ''
		&& is_int($arResult['ENTITY_ID'])
		&& $arResult['ENTITY_ID'] > 0
	)
	{
		$showAddEventButton = in_array(
			$arResult['ENTITY_TYPE'],
			[
		...
		{
			$showAddEventButton = !Container::getInstance()->getFactory(CCrmOwnerType::ResolveID($arResult['ENTITY_TYPE']))
				->getItemCategoryId($arResult['ENTITY_ID'])
			;
		}

		if (LayoutSettings::getCurrent()->isSliderEnabled() && $showAddEventButton)
		{
