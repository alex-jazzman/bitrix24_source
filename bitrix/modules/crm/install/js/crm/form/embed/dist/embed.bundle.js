...
(function (exports,main_qrcode,ui_stepbystep,ui_notification,landing_ui_field_color,ui_switcher,ui_feedback_form,main_loader,popup,ui_sidepanel_layout,main_core_events,ui_alerts,main_core) {
	...
	function handlerToggleCodeBlock(event) {
	  ...
	  if (main_core.Dom.style(blockCode, 'height') === "0px") {
	    ...
	    main_core.Dom.style(blockCode, "height", main_core.Text.encode(blockCode.scrollHeight) + "px"); // blockCode.clientHeight;
	...
	function _handleSwitcher2(formId, widgetId) {
	  ...
	  const formTypeOld = dataSetOld.formType; // get values for current form

	  const dataSetCurrent = this.getNode().closest('.crm-form-embed-widgets').dataset;
	  const formName = dataSetCurrent.formName;
	  const formType = dataSetCurrent.formType; // set related forms field to new values
	  ...
	  return BX.ajax.runAction('crm.form.assignWidgetToForm', {
	    ...
	    this.setLoading(false); // set to returned values (must match current)
	    ...
	    this.setLoading(false); // rollback on error
	...
	function _handleSwitcher2$1(formId, lineId) {
	  ...
	  const formNameOld = this.getNode().closest('.crm-form-embed-widgets-block').querySelector('.crm-form-embed-widgets-detail').dataset.formName; // get values for current form

	  const formName = this.getNode().closest('.crm-form-embed-widgets').dataset.formName; // set related forms field to new values
	  ...
	  updateDependentFields(container, name, value) {
	    switch (name) {
	      ...
	      case 'button.plain':
	        ...
	        const containerLinkStyle = container.querySelector('[data-option="button.decoration"]'); // const containerFont = container.querySelector('[data-option="button.font"]');
	        ...
	        if (value === '1') {
	          ...
	          main_core.Dom.show(containerLinkPosition); // // Dom.hide(containerFont);
	          ...
	          main_core.Dom.show(containerButtonPosition); // // Dom.show(containerFont);
	          // containerFont?.querySelectorAll('.crm-form-embed__settings-main--option').forEach((node) => {
	...
	class Controls {
	  ...
	  static renderColorPicker(subtype, options) {
	     ...
	     */
	    const handler = function (event) {
	      if (this instanceof landing_ui_field_color.ColorField) // double events with different context
	        {
	...
	function handlerToggleClickMode(container, checked) {
	  ...
	  if (checked) {
	    ...
	    main_core.Dom.style(blockCode, 'height', blockCode.scrollHeight + "px"); // blockCode.clientHeight;
	...
	const Renderer = {
	  ...
	  renderFieldButtonAlign: function (container, value, options) {
	    ...
	    const handlerRight = () => handler('right'); // container.appendChild(Controls.renderOptionTitle(Renderer.getOptionKeyName('button.align')));
	  ...
	  renderFieldButtonColorBackground: function (container, value, options) {
	    ...
	    const handler = val => {
	      ...
	      options.callback(fields).then(() => {// Renderer.renderFieldButtonColorBackground(container, val, options);
	      });
	    };
	  ...
	  renderFieldButtonColorBackgroundHover: function (container, value, options) {
	    ...
	    const handler = val => {
	      options.callback([{
	        name: 'button.color.backgroundHover',
	        value: val
	      }]).then(() => {// Renderer.renderFieldButtonColorBackgroundHover(container, val, options);
	      });
	    };
	  ...
	  renderFieldButtonColorText: function (container, value, options) {
	    ...
	    const handler = val => {
	      ...
	      options.callback(fields).then(() => {// Renderer.renderFieldButtonColorText(container, val, options);
	      });
	    };
	  ...
	  renderFieldButtonColorTextHover: function (container, value, options) {
	    ...
	    const handler = val => {
	      options.callback([{
	        name: 'button.color.textHover',
	        value: val
	      }]).then(() => {// Renderer.renderFieldButtonColorTextHover(container, val, options);
	      });
	    };
	...
	function _renderAdvancedSettingsLayoutContent2() {
	  ...
	  main_core.Dom.append(this.renderStepperSection(stepperContent), container); // TODO field dependency
	...
	class Click extends Form {
	  ...
	  save() {
	    this.dataProvider.updateValues('click', { ...babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].getValues()
	    });
	    return super.save();
	  }
	  ...
	  renderButtonSettings() {
	    const enhancedMode = !main_core.Type.isUndefined(this.dataProvider.getValues('click')['button']) && !main_core.Type.isUndefined(this.dataProvider.getValues('click')['button']['use']) && this.dataProvider.getValues('click')['button']['use'] === '1'; // const linkMode =
	    ...
	    const containerPlainAndText = babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].renderControlContainer([babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].renderOption('button.plain', true), babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].renderOption('button.text', true)]); // const optionStyle = (linkMode)
	    ...
	    const rowButtonSettings = babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].renderRow([// this.#wizard.renderTitle(BX.Loc.getMessage('EMBED_SLIDER_SUBTITLE_BUTTON_SETTINGS'), true),
	    containerPlainAndText, // this.#wizard.renderTitle(BX.Loc.getMessage('EMBED_SLIDER_SUBTITLE_STYLE')),
	    // optionStyle,
	    ...
	    const rowDesign = babelHelpers.classPrivateFieldLooseBase(this, _wizard$1)[_wizard$1].renderRow([// this.#wizard.renderTitle(
	    ...
	    main_core_events.EventEmitter.subscribeOnce('SidePanel.Slider:onOpenComplete', event => {
	      if (event.target.getUrl() === "crm.webform:embed:" + this.formId + ":click:expert") {
	        ...
	        const stepperContent = [{
	          html: [{
	            header: {
	              title: BX.Loc.getMessage('EMBED_SLIDER_CLICK_STEP_1_TITLE') // hint: 'hint text',
	...
	class Publink extends Form {
	  ...
	  render() {
	    ...
	    if (!super.loaded) {
	      ...
	      main_core.Dom.append(this.renderCopySection(null, // BX.Loc.getMessage('EMBED_SLIDER_COPY_LINK'),
...
}((this.BX.Crm.Form = this.BX.Crm.Form || {}),BX,BX.UI,BX,BX.Landing.UI.Field,BX,BX,BX,BX,BX.UI.SidePanel,BX.Event,BX.UI,BX));
