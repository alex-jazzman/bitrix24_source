(function(){"use strict";BX.namespace("BX.CRM.Kanban");BX.CRM.Kanban.Item=function(t){this.grid=null;this.container=null;this.timer=null;this.popupTooltip=null;this.plannerCurrent=null;this.fieldsWrapper=null;this.badgesWrapper=null;this.footerWrapper=null;this.clientName=null;this.clientNameItems=[];this.useAnimation=false;this.isAnimationInProgress=false;this.changedInPullRequest=false;this.notChangeTotal=false;this.itemActivityZeroClass="crm-kanban-item-activity-zero";this.activityAddingPopup=null;this.ufTooltipNodes=[];BX.Kanban.Item.apply(this,arguments)};BX.CRM.Kanban.Item.prototype={__proto__:BX.Kanban.Item.prototype,constructor:BX.CRM.Kanban.Item,lastPosition:{columnId:null,targetId:null},checked:false,setOptions(t){if(!t){return}BX.Kanban.Item.prototype.setOptions.call(this,t);this.useAnimation=BX.type.isBoolean(t.useAnimation)?t.useAnimation:false},setDataKey(t,e){const i=this.getData();i[t]=e;this.setData(i)},getDataKey(t){const e=this.getData();return e[t]},switchClass(t,e,i){if(i){BX.addClass(t,e)}else{BX.removeClass(t,e)}},switchVisible(t,e){if(e){BX.Dom.style(t,{display:""})}else{BX.Dom.style(t,{display:"none"})}},getLastPosition(){return this.lastPosition},setLastPosition(){const t=this.getColumn();const e=t.getNextItemSibling(this);this.lastPosition={columnId:t.getId(),targetId:e?e.getId():0}},getBodyContainer(){if(!this.layout.bodyContainer){this.layout.bodyContainer=BX.Tag.render`<div class="main-kanban-item-wrapper"></div>`}return this.layout.bodyContainer},render(){const t=this.getData();const e=t.special_type??null;if(e==="import"){return this.getPreparedStartLayout()}if(e==="rest"){return this.getPreparedIndustrySolutionsLayout()}if(!this.container){this.createLayout()}if(this.isLayoutFooterEveryRender()){this.layoutFooter()}this.setBorderColor();this.setLink();this.setPriceFormattedHtml();this.date.textContent=t.date;this.setClientName();if(this.planner){this.switchPlanner()}this.prepareContactTypeElements();this.appendLastActivity(t);if(this.needRenderFields()){this.fieldsWrapper.innerHTML=null;this.layoutFields()}this.layoutBadges();return this.container},getPreparedStartLayout(){const t=this.getStartLayout();this.emitOnSpecialItemDraw(t);this.grid.ccItem=this;BX.Dom.style(this.getBodyContainer(),{background:"none"});return t},getStartLayout(){this.getCloseStartLayout();const t=this.getGridData();const e=BX.Loc.getMessage("CRM_KANBAN_EMPTY_CARD_CT_TITLE");const i=BX.Loc.getMessage(`CRM_KANBAN_EMPTY_CARD_CT_TEXT${t.entityType}`);const n=t.rights.canImport?BX.Tag.render`
		...
		`},getPreparedIndustrySolutionsLayout(){const t=this.getIndustrySolutionsLayout();this.emitOnSpecialItemDraw(t);this.grid.restItem=this;return t},getIndustrySolutionsLayout(){const t=["CRM_KANBAN_REST_DEMO_FILE_IMPORT","CRM_KANBAN_REST_DEMO_FILE_EXPORT","CRM_KANBAN_REST_DEMO_CRM_MIGRATION","CRM_KANBAN_REST_DEMO_MARKET_2_MSGVER_1","CRM_KANBAN_REST_DEMO_PUBLICATION_2_MSGVER_1"];const e=document.createDocumentFragment();t.forEach(((t,i)=>{const n=`crm-kanban-item-industry-list-item crm-kanban-item-industry-list-item-${i+1}`;const s=BX.Loc.getMessage(t);const a=BX.Tag.render`
				<div class="${n}">
		...
		`},onIndustryCloseButtonClick(t){t.stopPropagation(t);this.getGrid().toggleRest();this.getGrid().registerAnalyticsSpecialItemCloseEvent(this,BX.Crm.Integration.Analytics.Dictionary.SUB_SECTION_KANBAN,BX.Crm.Integration.Analytics.Dictionary.ELEMENT_CLOSE_BUTTON,BX.Crm.Integration.Analytics.Dictionary.TYPE_ITEM_INDUSTRY)},emitOnSpecialItemDraw(t){BX.onCustomEvent("Crm.Kanban.Grid:onSpecialItemDraw",[this,t])},setBorderColor(){const t=this.getColumn().getColor();const e=BX.util.hex2rgb(t);const i=`rgba(${e.r},${e.g},${e.b},.7)`;BX.Dom.style(this.container,{"--crm-kanban-item-color":i})},setLink(){const t=this.getData();let e=this.clipTitle(t.name);if(t.isAutomationDebugItem){const t=BX.Loc.getMessage("CRM_KANBAN_ITEM_DEBUG_TITLE_MSGVER_1");e=`<span class="crm-kanban-debug-item-label">${t}</span> ${e}`}this.link.innerHTML=e;BX.Dom.attr(this.link,{href:t.link})},setPriceFormattedHtml(){const t=this.getData();if(this.totalPrice){this.totalPrice.innerHTML=t.price_formatted}},clipTitle(t){const e=" ";const i=t.split(e);const n=i.length-1;const s=`<span>${i[n]}</span>`;i.splice(n);return`${i.join(e)}${e}${s}`},setClientName(){const t=this.getData();const e=this.getGridData();this.clientNameItems=[];if(this.getContactId()&&t.contactName&&e.customFields.includes("CLIENT")){this.clientNameItems.push(t.contactTooltip)}if(this.getCompanyId()&&t.companyName&&e.customFields.includes("CLIENT")){this.clientNameItems.push(t.companyTooltip)}if(BX.Type.isArrayFilled(this.clientNameItems)){this.clientName.innerHTML="";this.clientNameItems.forEach((t=>{if(!t.includes('data-mini-card="true"')){return}const e=BX.Tag.render`${t}`;const i=Number(e.dataset.entityTypeId);const n=Number(e.dataset.entityId);BX.Runtime.loadExtension("crm.mini-card").then((({EntityMiniCard:t})=>{new BX.Crm.EntityMiniCard({bindElement:e,entityTypeId:i,entityId:n})}));BX.Dom.append(e,this.clientName)}));this.switchVisible(this.clientName,true)}else{this.switchVisible(this.clientName,false)}},prepareContactTypeElements(){const t=this.getData();const e=["Phone","Email","Im"];e.forEach((e=>{const i=`contact${e}`;BX.Event.unbindAll(this[i]);const n=`crm-kanban-item-contact-${e.toLowerCase()}-disabled`;if(t[e.toLowerCase()]){BX.Event.bind(this[i],"click",(t=>{this.clickContact(e.toLowerCase(),t.target)}));this.switchClass(this[i],n,false);return}BX.Event.bind(this[i],"mouseover",(({target:t})=>{const e=BX.Dom.attr(t,"data-type");this.showTooltip(BX.Loc.getMessage(`CRM_KANBAN_NO_${e.toUpperCase()}`),t)}));BX.Event.bind(this[i],"mouseout",this.hideTooltip.bind(this));this.switchClass(this[i],n,true)}))},appendLastActivity(t){if(!this.isShowLastActivityTime()&&!this.isShowLastActivityUser()){return}if(this.isShowLastActivityTime()){BX.Dom.clean(this.lastActivityTime)}else{BX.Dom.remove(this.lastActivityTime)}if(this.isShowLastActivityUser()){BX.Dom.clean(this.lastActivityBy)}else{BX.Dom.remove(this.lastActivityBy)}const e=t.lastActivity;if(!BX.Type.isPlainObject(e)||!BX.CRM.Kanban.Restriction.Instance.isLastActivityInfoInKanbanItemAvailable()){return}if(this.isShowLastActivityTime()){this.appendLastActivityTime(e)}if(this.isShowLastActivityUser()){this.appendLastActivityUser(e)}},appendLastActivityTime(t){const e=BX.Text.toInteger(t.timestamp);if(e<=0){return}const i=e-BX.Main.Timezone.Offset.USER_TO_SERVER;const n=BX.Main.Timezone.UserTime.getDate(i);const s=BX.Main.Timezone.UserTime.getDate();const a=(s.getTime()-n.getTime())/1e3;const r=a<=60?BX.Text.encode(BX.Loc.getMessage("CRM_KANBAN_JUST_NOW")):this.getFormattedLastActiveDateTime(n,s);const o=BX.Tag.render`
		...
		`;BX.Dom.append(d,this.fieldsWrapper);BX.UI.Hint.init(BX(this.fieldsWrapper))},getFieldConfig(t){const e=this.getFieldsConfig().find((e=>t.code===e.code))??t;if(e.type==="user"){e.value=this.getUserConfigById(t.value)??t.value}return e},getFieldsConfig(){return this.getItemsConfigData().fields??[]},getItemsConfigData(){return this.getGridData().itemsConfig??{}},getFieldParams(t){const e=t.type??"string";let i={props:{className:"crm-kanban-item-fields-item-value"}};if(e==="user"){i={...i,...this.getUserTypeFieldParams(t)}}else if(t.value.includes('data-mini-card="true"')||BX.Type.isArray(t.value)&&t.value.every((t=>t.includes('data-mini-card="true"')))){const e=BX.Type.isArray(t.value)?t.value:[t.value];const n=BX.Tag.render`<div class="crm-mini-card-list-container"></div>`;e.forEach((t=>{const e=BX.Tag.render`${t}`;const i=Number(e.dataset.entityTypeId);const s=Number(e.dataset.entityId);void BX.Runtime.loadExtension("crm.mini-card").then((({EntityMiniCard:t})=>{new t({bindElement:e,entityTypeId:i,entityId:s})}));BX.Dom.append(e,n)}));i.children=[n]}else if(t.type==="money"||t.html===true){const e=t.valueDelimiter??"<br>";i.html=BX.Type.isArray(t.value)?t.value.join(e):t.value;if(i.html.includes("<b>")){i.props.className=`${i.props.className} --normal-weight`}}else{i.text=BX.Type.isArray(t.value)?t.value.join(", "):t.value}return i},getUserTypeFieldParams(t){const e={};if(!BX.Type.isPlainObject(t.value)||BX.Type.isArray(t.value)){if(BX.Type.isArray(t.value)){const i=[];t.value.forEach((t=>{const e=this.getItemsConfigData().users?.find((e=>e.id===Number(t)))??null;if(BX.Type.isPlainObject(e)){const t=this.getInfoFromUserTypeFieldValue(e);i.push(t.balloon)}}));e.html=i.join(", ");return e}const i=this.getItemsConfigData().users?.find((e=>e.id===Number(t.value)))??null;const n=this.getInfoFromUserTypeFieldValue(i);e.html=n.balloon}if(t.html!==true){e.text=this.getMessage("noname");return e}if(BX.Type.isPlainObject(t.value)){const i=this.getInfoFromUserTypeFieldValue(t.value);if(t.code==="ASSIGNED_BY_ID"){e.html=`\n\t\t\t\t\t<div class="crm-kanban-item-fields-item-value-user">\n\t\t\t\t\t\t${i.picture}\n\t\t\t\t\t\t${i.name}\n\t\t\t\t\t</div>\n\t\t\t\t`}else{const t=i.balloon??`${i.picture}\n${i.name}`;e.html=`\n\t\t\t\t\t<div class="crm-kanban-item-fields-item-value-user">\n\t\t\t\t\t\t${t}\n\t\t\t\t\t</div>\n\t\t\t\t`}}else{e.html=BX.Type.isArray(t.value)?t.value.join(", "):t.value}return e},getInfoFromUserTypeFieldValue(t){if(!BX.Type.isPlainObject(t)){return{}}let e="";let i="";if(t.link===""){e='<span class="crm-kanban-item-fields-item-value-userpic"></span>';i=`<span class="crm-kanban-item-fields-item-value-name">${t.title}</span>`}else{let n="";if(t.picture){n=` style="background-image: url('${encodeURI(BX.Text.encode(t.picture))}')"`}const s=t.id?`bx-tooltip-user-id="${BX.Text.toInteger(t.id)}" bx-tooltip-context="b24"`:null;e=`<a class="crm-kanban-item-fields-item-value-userpic" ${s} href="${t.link}"${n}></a>`;i=`<a class="crm-kanban-item-fields-item-value-name" ${s} href="${t.link}">${t.title}</a>`}return{picture:e,name:i,balloon:t.balloon}},layoutBadges(){BX.Dom.clean(this.badgesWrapper);const t=this.getBadges();for(let e=0;e<t.length;e++){const i=t[e];const n="crm-kanban-item-badges-item-value crm-kanban-item-badges-status";const s=`\n\t\t\t\tbackground-color: ${i.backgroundColor};\n\t\t\t\tborder-color: ${i.backgroundColor};\n\t\t\t\tcolor: ${i.textColor};\n\t\t\t`;const a=BX.Tag.render`
			...
			></div>
		`;BX.Event.bind(this.container,"animationend",(()=>{BX.Dom.removeClass(this.layout.container,"main-kanban-item-new")}));return this.container},onContainerClick(t){const e=t.target;const i=this.container.className.replace(" ",".");const n=e.closest(`.${i}`);if(e!==this.container&&!n||n&&e.tagName==="A"||n&&e.tagName==="SPAN"&&!BX.Dom.hasClass(e,"crm-kanban-item-contact")){return}const s=this.getGrid();if(this.checked){s.unCheckItem(this);if(!BX.Type.isArrayFilled(s.getChecked())){s.resetMultiSelectMode();s.stopActionPanel()}}else{s.checkItem(this);s.onMultiSelectMode();s.startActionPanel()}s.calculateTotalCheckItems()},onContainerDblClick(){this.link.click()},onContainerMouseLeave(){this.hideUfTooltips();this.removeHoverClass(this.container)},onContainerMouseEnter(){this.showUfTooltips()},hideUfTooltips(){this.ufTooltipNodes.forEach(((t,e)=>{this.ufTooltipNodes[e].classList.add("--hidden")}))},showUfTooltips(){this.ufTooltipNodes.forEach(((t,e)=>{this.ufTooltipNodes[e].classList.remove("--hidden")}))},createTitleLink(){this.link=BX.Tag.render`<a class="crm-kanban-item-title" style="${this.getBlockStyleBasedOnFields()}"></a>`;return this.link},createLine(){return BX.Tag.render`<div class="crm-kanban-item-line"></div>`},createRepeated(){const t=this.options.data;if(!t.return&&!t.returnApproach){return null}const e=this.getGridData().entityType;const i=t.returnApproach?BX.Loc.getMessage(`CRM_KANBAN_REPEATED_APPROACH_${e}`):BX.Loc.getMessage(`CRM_KANBAN_REPEATED_${e}`);return BX.Tag.render`<div class="crm-kanban-item-repeated">${i}</div>`},createTotalPrice(){this.totalPrice=BX.Tag.render`<div class="crm-kanban-item-total-price"></div>`;this.total=BX.Tag.render`
