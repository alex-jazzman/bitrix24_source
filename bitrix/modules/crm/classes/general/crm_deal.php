...
class CAllCrmDeal
{
	...
	public static function GetFields($arOptions = null)
	{
		...
		$result = array(
			...
			'LAST_ACTIVITY_BY' => ['FIELD' => 'L.LAST_ACTIVITY_BY', 'TYPE' => 'int'],
			'LAST_ACTIVITY_TIME' => ['FIELD' => 'L.LAST_ACTIVITY_TIME', 'TYPE' => 'datetime'],
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			$arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_TIME] = $arFields['DATE_CREATE'] ?? (new \Bitrix\Main\Type\DateTime())->toString();
			$arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_BY] = $arFields['CREATED_BY_ID'] ?? $userID;
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			unset($arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_TIME], $arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_BY]);

			$monitor = Crm\Service\Timeline\Monitor::getInstance();
			$identifier = new Crm\ItemIdentifier(\CCrmOwnerType::Deal, (int)$ID);

			if ($monitor->isTimelineChanged($identifier))
			{
				/** @var \Bitrix\Main\Type\DateTime|null $lastActivityTime */
				/** @var int|null $lastActivityBy */
				[$lastActivityTime, $lastActivityBy] = $monitor->calculateLastActivityInfo($identifier);

				if ($lastActivityTime)
				{
					$lastActivityTime = (string)$lastActivityTime;
				}
				else
				{
					$lastActivityTime = $arRow['DATE_CREATE'];
				}

				$arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_TIME] = $lastActivityTime;
				$arFields[Crm\Item::FIELD_NAME_LAST_ACTIVITY_BY] = $lastActivityBy ?? $arRow['CREATED_BY_ID'];
			}
