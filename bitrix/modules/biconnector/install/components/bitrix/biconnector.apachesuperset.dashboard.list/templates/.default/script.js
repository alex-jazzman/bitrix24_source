/* eslint-disable */
(function (exports,main_core,main_popup,main_date,biconnector_apacheSupersetDashboardManager,main_core_events,ui_dialogs_messagebox,biconnector_apacheSupersetAnalytics,ui_entitySelector,ui_tour,biconnector_apacheSupersetMarketManager,biconnector_entitySelector,ui_buttons) {
	'use strict';
	  ...
	  babelHelpers.createClass(SupersetDashboardGridManager, [{
	    ...
	    value: function duplicateDashboard(dashboardId) {
	      ...
	      return babelHelpers.classPrivateFieldGet(this, _dashboardManager).duplicateDashboard(dashboardId).then(function (response) {
	        ...
	        var firstDashboardRow = babelHelpers.classPrivateFieldGet(_this3, _grid).getRows().getRowsByGroupId('D').find(function (row) {
	        ...
	        var newRowNode = babelHelpers.classPrivateFieldGet(_this3, _grid).getRows().getById(newDashboard.id).node;
	    ...
	    value: function deleteGroup(groupId) {
	      var _this7 = this;
	      var messageBox = new ui_dialogs_messagebox.MessageBox({
	        message: main_core.Loc.getMessage('BICONNECTOR_SUPERSET_DASHBOARD_GRID_DELETE_GROUP_POPUP_TITLE'),
	        buttons: [new ui_buttons.Button({
	          color: ui_buttons.ButtonColor.DANGER,
	    ...
	    value: function renameDashboard(dashboardId) {
	      ...
	      var editor = _classPrivateMethodGet(this, _buildDashboardTitleEditor, _buildDashboardTitleEditor2).call(this, dashboardId, row.getEditData().TITLE, function () {
	        _this9.cancelRenameDashboard(dashboardId);
	      }, function (innerTitle) {
	        var oldTitle = _classPrivateMethodGet(_this9, _getTitlePreview, _getTitlePreview2).call(_this9, dashboardId).querySelector('a').innerText;
	        _classPrivateMethodGet(_this9, _getTitlePreview, _getTitlePreview2).call(_this9, dashboardId).querySelector('a').innerText = innerTitle;
	        ...
	        _this9.cancelRenameDashboard(dashboardId);
	        _classPrivateMethodGet(_this9, _setDateModifyNow, _setDateModifyNow2).call(_this9, dashboardId);
	        babelHelpers.classPrivateFieldGet(_this9, _dashboardManager).renameDashboard(dashboardId, innerTitle)["catch"](function (response) {
	          if (response.errors) {
	            _classPrivateMethodGet(_this9, _notifyErrors, _notifyErrors2).call(_this9, response.errors);
	          }
	          _classPrivateMethodGet(_this9, _getTitlePreview, _getTitlePreview2).call(_this9, dashboardId).querySelector('a').innerText = oldTitle;
	    ...
	    value: function handleTagAddClick(dashboardId, preselectedIds, event) {
	      ...
	      var onTagsChange = function onTagsChange() {
	        var tags = babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).getSelectedItems().map(function (item) {
	          return item.getId();
	        });
	        babelHelpers.classPrivateFieldGet(_this10, _dashboardManager).setDashboardTags(dashboardId, tags).then(function () {
	          var _filterTagValues$TAGS;
	          _this10.getGrid().updateRow(dashboardId, null, null, function () {
	            var _this10$getGrid$getRo;
	            var anchor = (_this10$getGrid$getRo = _this10.getGrid().getRows().getById(dashboardId)) === null || _this10$getGrid$getRo === void 0 ? void 0 : _this10$getGrid$getRo.getCellById('TAGS');
	            if (anchor && babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog)) {
	              babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).setTargetNode(anchor);
	            }
	          });
	          var filterTagValues = _this10.getFilter().getFilterFieldsValues();
	          ...
	          if (currentFilteredTags.length > 0) {
	            ...
	            if (filtered.length === 0) {
	              babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).destroy();
	              babelHelpers.classPrivateFieldSet(_this10, _tagSelectorDialog, null);
	            }
	          }
	        })["catch"](function (response) {
	          if (response.errors) {
	      ...
	      babelHelpers.classPrivateFieldSet(this, _tagSelectorDialog, new ui_entitySelector.Dialog({
	        ...
	        events: {
	          onSearch: function onSearch(event) {
	            ...
	            var footer = babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).getFooter();
	            var footerWrapper = babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).getFooterContainer();
	          ...
	          'Search:onItemCreateAsync': function SearchOnItemCreateAsync(searchEvent) {
	            return new Promise(function (resolve, reject) {
	              ...
	              babelHelpers.classPrivateFieldGet(_this10, _dashboardManager).addTag(name).then(function (result) {
	                var newTag = result.data;
	                var item = babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).addItem({
	                  id: newTag.ID,
	                  entityId: entityId,
	                  title: name,
	                  tabs: 'all'
	                ...
	                errors.forEach(function (error) {
	                  ...
	                  main_core.Dom.prepend(alert, babelHelpers.classPrivateFieldGet(_this10, _tagSelectorDialog).getFooterContainer());
	                  setTimeout(function () {
	    ...
	    value: function addToTopMenu(dashboardId, url) {
	      ...
	      return babelHelpers.classPrivateFieldGet(this, _dashboardManager).addToTopMenu(dashboardId).then(function (response) {})["catch"](function (response) {
	        babelHelpers.classPrivateFieldGet(_this11, _grid).updateRow(dashboardId);
	        BX.UI.Notification.Center.notify({
	...
	function _subscribeToEvents2() {
	  ...
	  main_core_events.EventEmitter.subscribe('SidePanel.Slider:onMessage', function (event) {
	    ...
	    if (sliderEvent.getEventId() === 'BIConnector.Superset.DashboardDetail:onDashboardBatchStatusUpdate') {
	      ...
	      if (eventArgs.dashboardList) {
	        _this15.onUpdatedDashboardBatchStatus(eventArgs.dashboardList);
	      }
	      ...
	      if (babelHelpers.classPrivateFieldGet(_this15, _tagSelectorDialog)) {
	        babelHelpers.classPrivateFieldGet(_this15, _tagSelectorDialog).destroy();
	        babelHelpers.classPrivateFieldSet(_this15, _tagSelectorDialog, null);
	      }
	      var filterTagValues = _this15.getFilter().getFilterFieldsValues();
	      if (main_core.Type.isUndefined(filterTagValues['TAGS.ID']) || filterTagValues['TAGS.ID'].length === 0) {
	        _this15.getGrid().reload();
	        return;
	      }
	      ...
	      var filterApi = _this15.getFilter().getApi();
	  ...
	  main_core_events.EventEmitter.subscribe('BIConnector.Superset.DashboardManager:onDashboardBatchStatusUpdate', function (event) {
	    ...
	    _this15.onUpdatedDashboardBatchStatus(dashboardList);
	  });
	  ...
	  main_core_events.EventEmitter.subscribe('onPullEvent-biconnector', function (event) {
	    ...
	    if (eventName === 'onSupersetStatusUpdated') {
	      ...
	      if (status) {
	        _classPrivateMethodGet(_this15, _onSupersetStatusChange, _onSupersetStatusChange2).call(_this15, status);
	      }
	  ...
	  main_core_events.EventEmitter.subscribe('Grid::updated', function () {
	    _classPrivateMethodGet(_this15, _initHints, _initHints2).call(_this15);
	    _classPrivateMethodGet(_this15, _colorPinnedRows, _colorPinnedRows2).call(_this15);
	  });
	  main_core_events.EventEmitter.subscribe('BIConnector.ExportMaster:onDashboardDataLoaded', function () {
	    babelHelpers.classPrivateFieldGet(_this15, _grid).tableUnfade();
	  });
	  ...
	  main_core_events.EventEmitter.subscribe('BIConnector.CreateForm:onDashboardCreated', function (event) {
	    _classPrivateMethodGet(_this15, _onNewDashboardCreated, _onNewDashboardCreated2).call(_this15, event);
	  });
	...
	function _onNewDashboardCreated2(event) {
	  var _this$getFilter$getFi,
	    _this16 = this;
	  var newDashboard = event.data.dashboard;
	  var needAddRowInRealtime = false;
	  var filterGroupIds = (_this$getFilter$getFi = this.getFilter().getFilterFieldsValues()['GROUPS.ID']) !== null && _this$getFilter$getFi !== void 0 ? _this$getFilter$getFi : [];
	  if (filterGroupIds.length === 0 && newDashboard.groupIds.length === 0) {
	    needAddRowInRealtime = true;
	  } else {
	    var _iterator5 = _createForOfIteratorHelper(newDashboard.groupIds),
	      _step5;
	    try {
	      for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
	        var dashboardGroupId = _step5.value;
	  ...
	  if (!needAddRowInRealtime) {
	    BX.UI.Notification.Center.notify({
	      content: main_core.Loc.getMessage('BICONNECTOR_SUPERSET_DASHBOARD_GRID_DASHBOARD_CREATED')
	    });
	    return;
	  }
	  var grid = this.getGrid();
	  ...
	  var newRowNode = babelHelpers.classPrivateFieldGet(this, _grid).getRows().getById(newDashboard.id).node;
	  newRowNode.setAttribute('data-group-id', 'D');
	  var editableData = grid.getParam('EDITABLE_DATA');
	  if (BX.type.isPlainObject(editableData)) {
	    editableData[newDashboard.id] = {
	  ...
	  _classPrivateMethodGet(this, _initHints, _initHints2).call(this);
	  setTimeout(function () {
	    _classPrivateMethodGet(_this16, _showDraftGuide, _showDraftGuide2).call(_this16, newRowNode);
	  }, 1200);
	}
...
}((this.window = this.window || {}),BX,BX.Main,BX.Main,BX.BIConnector,BX.Event,BX.UI.Dialogs,BX.BIConnector,BX.UI.EntitySelector,BX.UI.Tour,BX.BIConnector,BX.BIConnector.EntitySelector,BX.UI));
