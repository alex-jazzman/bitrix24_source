this.BX=this.BX||{};this.BX.BIConnector=this.BX.BIConnector||{};(function(t,e,i,s,n,o,a,r,l,d,c,p,u,h,g,m,f,_,T,E,v,S){"use strict";const P={props:{saveLocked:{type:Boolean,required:false,default:false},isEditMode:{type:Boolean,required:false,default:false}},computed:{saveButtonClass(){return{"ui-btn-base-light":this.saveLocked,"app-root__button--blocked":this.saveLocked,"ui-btn-success":!this.saveLocked}},saveButtonText(){return this.isEditMode?this.$Bitrix.Loc.getMessage("DATASET_IMPORT_SAVE"):this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CREATE")}},mounted(){v.Dom.addClass(document.querySelector("body"),"app-body");const t=this.$refs.buttonsPanel;if(t){this.$root.$el.parentNode.appendChild(t);new BX.UI.Pinner(t,{fixBottom:true,fullWidth:true})}},methods:{onCreateButtonClick(){this.$Bitrix.eventEmitter.emit("biconnector:dataset-import:createButtonClick",{})},onCancelButtonClick(){this.$Bitrix.eventEmitter.emit("biconnector:dataset-import:cancelButtonClick",{})}},template:`\n\t\t<div class="app">\n\t\t\t<div class="app__block app__block--narrow">\n\t\t\t\t<slot name="left-panel"></slot>\n\t\t\t</div>\n\t\t\t<div class="app__block">\n\t\t\t\t<slot name="right-panel"></slot>\n\t\t\t</div>\n\t\n\t\t\t<div class="ui-button-panel-wrapper" ref="buttonsPanel">\n\t\t\t\t<div class="ui-button-panel">\n\t\t\t\t\t<div class="app-root__button-wrapper" :class="saveLocked ? 'app-root__button-wrapper--blocked' : ''">\n\t\t\t\t\t\t<button class="ui-btn ui-btn-md app-root__button" :class="saveButtonClass" @click="onCreateButtonClick" ref="saveButton">\n\t\t\t\t\t\t\t{{ saveButtonText }}\n\t\t\t\t\t\t</button>\n\t\t\t\t\t</div>\n\t\t\t\t\t<button class="ui-btn ui-btn-md ui-btn-link app-root__button" @click="onCancelButtonClick">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CANCEL') }}\n\t\t\t\t\t</button>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</div>\n\t`};const b={template:`\n\t\t<div class="import-config">\n\t\t\t<slot></slot>\n\t\t</div>\n\t`};const I={emits:["close"],props:{id:{type:String,required:true},options:{type:Object,required:false,default:{}},wrapperClass:{type:String,required:false,default:""}},data(){return{popupInstance:null}},computed:{popupOptions(){return{id:this.id,content:this.$refs.content,autoHide:true,events:{onPopupClose:this.closePopup,onPopupDestroy:this.closePopup},fixed:true,...this.options}}},methods:{closePopup(){var t;(t=u.PopupManager.getPopupById(this.id))==null?void 0:t.destroy();this.popupInstance=null;this.$emit("close")}},mounted(){if(!this.popupInstance){this.popupInstance=new u.Popup(this.popupOptions)}this.popupInstance.show()},beforeUnmount(){this.closePopup()},template:`\n\t\t<div ref="content" :class="wrapperClass">\n\t\t\t<slot></slot>\n\t\t</div>\n\t`};const A={emits:["close","saveIgnoringErrors"],props:{errors:{type:Array,required:true},isEditMode:{type:Boolean,required:true},isSavingMode:{type:Boolean,required:true}},data(){return{reportDownloadLink:null}},computed:{popupOptions(){return{width:830,closeIcon:false,noAllPaddings:true,overlay:true}},title(){if(!this.isSavingMode){if(this.errors.length>=200){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_TITLE_MANY")}return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_TITLE")}if(this.errors.length<=200){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_TITLE")}return this.isEditMode?this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_TITLE_STOPPED_UPDATING"):this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_TITLE_STOPPED_CREATING")},hintContent(){if(this.errors.length>200){if(this.isEditMode){return this.prepareHintContent("DATASET_IMPORT_CSV_ERROR_POPUP_HINT_UPDATING_MANY_ERRORS")}return this.prepareHintContent("DATASET_IMPORT_CSV_ERROR_POPUP_HINT_CREATION_MANY_ERRORS")}if(this.isSavingMode){if(this.isEditMode){return this.prepareHintContent("DATASET_IMPORT_CSV_ERROR_POPUP_HINT_UPDATING")}return this.prepareHintContent("DATASET_IMPORT_CSV_ERROR_POPUP_HINT_CREATION")}return this.prepareHintContent("DATASET_IMPORT_CSV_ERROR_POPUP_HINT_CHECKING")},errorsCountText(){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_SUMMARY").replace("[wrapper]",'<span class="errors-counter">').replace("#COUNT#",this.errors.length>200?"200+":this.errors.length).replace("[/wrapper]","</span>")}},methods:{onClose(){this.$emit("close")},onIgnoreErrorsClick(){this.$emit("saveIgnoringErrors")},onDownloadLogButtonClick(){this.downloadLogButton.setState(g.ButtonState.WAITING);if(this.reportDownloadLink){this.downloadLogButton.setState(null);this.downloadLog();return}v.ajax.runAction("biconnector.externalsource.dataset.logErrorsIntoFile",{data:{fields:this.$store.state.config,type:"csv"}}).then((t=>{this.downloadLogButton.setState(null);const e=new Blob([t.data],{type:"text/html"});this.reportDownloadLink=window.URL.createObjectURL(e);this.downloadLog()})).catch((()=>{this.downloadLogButton.setState(null)}))},downloadLog(){const t=document.createElement("a");t.href=this.reportDownloadLink;t.download="import_errors.html";v.Dom.append(t,document.body);t.click();v.Dom.remove(t)},prepareHintContent(t){const e=this.$Bitrix.Loc.getMessage(t);return e.replace("[link]",'<a onclick="top.BX.Helper.show(`redirect=detail&code=23779844`)">').replace("[/link]","</a>")}},mounted(){this.downloadLogButton=new g.Button({size:g.ButtonSize.EXTRA_SMALL,color:g.ButtonColor.LIGHT,noCaps:true,onclick:this.onDownloadLogButtonClick,text:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_CSV_ERROR_POPUP_DOWNLOAD_LOG"),className:"popup-errors-log-button"});this.downloadLogButton.renderTo(this.$refs.summaryBlock)},beforeUnmount(){window.URL.revokeObjectURL(this.reportDownloadLink);this.reportDownloadLink=null},components:{Popup:I},template:`\n\t\t<Popup id="generic" @close="this.onClose" :options="popupOptions" wrapper-class="errors-popup">\n\t\t\t<h3 class="popup-header">{{ title }}</h3>\n\t\t\t<div class="ui-alert ui-alert-primary popup-hint">\n\t\t\t\t<span v-html="hintContent"></span>\n\t\t\t</div>\n\n\t\t\t<div class="popup-errors-summary" ref="summaryBlock">\n\t\t\t\t<div\n\t\t\t\t\tclass="popup-errors-count"\n\t\t\t\t\tv-html="errorsCountText"\n\t\t\t\t></div>\n\t\t\t</div>\n\n\t\t\t<div \n\t\t\t\tclass="popup-error-header-wrapper"\n\t\t\t\t:style="errors.length >= 6 ? {'padding-right': '13px'} : {}"\n\t\t\t>\n\t\t\t\t<table class="popup-error-header">\n\t\t\t\t\t<tbody>\n\t\t\t\t\t<tr>\n\t\t\t\t\t<td class="popup-error-header-cell cell-error-number">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_TABLE_HEADER_ERROR_NUMBER') }}\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class="popup-error-header-cell cell-error-message">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_TABLE_HEADER_ERROR_MESSAGE') }}\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class="popup-error-header-cell cell-error-line">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_TABLE_HEADER_LINE') }}\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class="popup-error-header-cell cell-error-column-name">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_TABLE_HEADER_COL_NAME') }}\n\t\t\t\t\t</td>\n\t\t\t\t\t</tr>\n\t\t\t\t\t</tbody>\n\t\t\t\t</table>\n\t\t\t</div>\n\n\t\t\t<div \n\t\t\t\tclass="popup-error-table-wrapper"\n\t\t\t\t:style="errors.length >= 6 ? {'padding-right': '3px'} : {}"\n\t\t\t>\n\t\t\t\t<div class="popup-error-table-scroll">\n\t\t\t\t\t<table class="popup-error-table">\n\t\t\t\t\t\t<tr v-for="(error, index) in errors">\n\t\t\t\t\t\t\t<template v-if="index < 200">\n\t\t\t\t\t\t\t\t<td class="popup-error-table-row-cell cell-error-number">{{ index + 1 }}</td>\n\t\t\t\t\t\t\t\t<td class="popup-error-table-row-cell cell-error-message">{{ error.errorMessage }}</td>\n\t\t\t\t\t\t\t\t<td class="popup-error-table-row-cell cell-error-line">{{ error.lineNumber }}</td>\n\t\t\t\t\t\t\t\t<td class="popup-error-table-row-cell cell-error-column-name" :title="error.columnName">{{ error.columnName }}</td>\n\t\t\t\t\t\t\t</template>\n\t\t\t\t\t\t</tr>\n\t\t\t\t\t</table>\n\t\t\t\t</div>\n\t\t\t</div>\n\n\t\t\t<div class="popup-buttons-wrapper">\n\t\t\t\t<template v-if="isSavingMode">\n\t\t\t\t\t<template v-if="errors.length > 200">\n\t\t\t\t\t\t<button @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary">\n\t\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_BUTTON_CLOSE') }}\n\t\t\t\t\t\t</button>\n\t\t\t\t\t</template>\n\t\t\t\t\t<template v-else>\n\t\t\t\t\t\t<button v-if="isEditMode" @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary">\n\t\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_BUTTON_STOP_UPDATE') }}\n\t\t\t\t\t\t</button>\n\t\t\t\t\t\t<button v-else @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary">\n\t\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_BUTTON_STOP') }}\n\t\t\t\t\t\t</button>\n\n\t\t\t\t\t\t<button @click="this.onIgnoreErrorsClick" class="ui-btn ui-btn-md ui-btn-light-border">\n\t\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_BUTTON_IGNORE') }}\n\t\t\t\t\t\t</button>\n\t\t\t\t\t</template>\n\t\t\t\t</template>\n\t\t\t\t<template v-else>\n\t\t\t\t\t<button @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary">\n\t\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_BUTTON_CLOSE') }}\n\t\t\t\t\t</button>\n\t\t\t\t</template>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const C={emits:["close"],props:{},computed:{popupOptions(){return{width:440,closeIcon:false,noAllPaddings:true,overlay:true,background:"transparent"}}},mounted(){const t=new c.Loader({target:this.$refs.loader,size:65,color:"var(--ui-color-primary)",strokeWidth:4,mode:"inline"});t.show()},methods:{onClose(){this.$emit("close")}},components:{Popup:I},template:`\n\t\t<Popup\n\t\t\tid="file-check-progress"\n\t\t\t@close="this.onClose"\n\t\t\t:options="popupOptions"\n\t\t>\n\t\t\t<div class="file-check file-check-progress">\n\t\t\t\t<div ref="loader" class="file-check-loader"></div>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const M={emits:["close"],props:{},computed:{popupOptions(){return{width:440,closeIcon:true,noAllPaddings:true,overlay:true}}},methods:{onClose(){this.$emit("close")}},components:{Popup:I},template:`\n\t\t<Popup \n\t\t\tid="file-check-success"\n\t\t\t@close="this.onClose" \n\t\t\t:options="popupOptions" \n\t\t>\n\t\t\t<div class="file-check file-check-success">\n\t\t\t\t<div class="file-check-icon-success"></div>\n\t\t\t\t<div class="file-check-title">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_OK_TITLE') }}</div>\n\t\t\t\t<div class="file-check-subtitle">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_OK_SUBTITLE') }}</div>\n\t\t\t\t<button @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary file-check-button">\n\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_OK_BUTTON') }}\n\t\t\t\t</button>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const D={emits:["close"],props:{},computed:{popupOptions(){return{width:440,closeIcon:true,noAllPaddings:true,overlay:true}}},methods:{onClose(){this.$emit("close")}},components:{Popup:I},template:`\n\t\t<Popup \n\t\t\tid="file-check-failed"\n\t\t\t@close="this.onClose" \n\t\t\t:options="popupOptions" \n\t\t>\n\t\t\t<div class="file-check file-check-failed">\n\t\t\t\t<div class="file-check-icon-failed"></div>\n\t\t\t\t<div class="file-check-title">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_FAILED_TITLE') }}</div>\n\t\t\t\t<div class="file-check-subtitle">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_FAILED_SUBTITLE') }}</div>\n\t\t\t\t<button @click="this.onClose" class="ui-btn ui-btn-md ui-btn-primary file-check-button">\n\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_CSV_ERROR_POPUP_CHECK_FAILED_BUTTON') }}\n\t\t\t\t</button>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const O={emits:["close"],props:{title:{type:String,required:true}},computed:{popupOptions(){return{width:440,closeIcon:true,noAllPaddings:true,overlay:true}}},methods:{onClose(){this.$emit("close")}},components:{Popup:I},template:`\n\t\t<Popup id="generic" @close="this.onClose" :options="popupOptions" wrapper-class="generic-popup">\n\t\t\t<h3 class="generic-popup__header">{{ title }}</h3>\n\t\t\t<div class="generic-popup__content">\n\t\t\t\t<slot name="content"></slot>\n\t\t\t</div>\n\t\t\t<div class="generic-popup__buttons-wrapper">\n\t\t\t\t<slot name="buttons"></slot>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const y={emits:["close"],props:{title:{type:String,required:true},description:{type:String,required:false,default:""},options:{type:Object,required:false,default:{}}},computed:{popupOptions(){return{width:500,minHeight:299,closeIcon:true,noAllPaddings:true,overlay:true,...this.options}}},methods:{onClose(){this.$emit("close")}},components:{Popup:I},template:`\n\t\t<Popup id="saveProgress" @close="this.onClose" :options="popupOptions" wrapper-class="dataset-import-popup--full-height">\n\t\t\t<div class="dataset-save-progress-popup">\n\t\t\t\t<div class="dataset-save-progress-popup__content">\n\t\t\t\t\t<slot name="icon"></slot>\n\t\t\t\t\t<div class="dataset-save-progress-popup__texts">\n\t\t\t\t\t\t<h3 class="dataset-save-progress-popup__header">{{ title }}</h3>\n\t\t\t\t\t\t<p class="dataset-save-progress-popup__description" v-if="description">{{ description }}</p>\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class="dataset-save-progress-popup__buttons"><slot name="buttons"></slot></div>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</Popup>\n\t`};const R={props:{title:{type:String,required:true},description:{type:String,required:true}},emits:["click"],computed:{},methods:{onClick(){this.$emit("click")}},components:{SaveProgressPopup:y},template:`\n\t\t<SaveProgressPopup\n\t\t\t:title="title"\n\t\t\t:description="description"\n\t\t>\n\t\t\t<template v-slot:icon>\n\t\t\t\t<div class="dataset-save-progress-popup__failure-logo"></div>\n\t\t\t</template>\n\t\t\t<template v-slot:buttons>\n\t\t\t\t<button @click="onClick" class="ui-btn ui-btn-md ui-btn-primary">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_FAILURE_POPUP_BUTTON') }}</button>\n\t\t\t</template>\n\t\t</SaveProgressPopup>\n\t`};const L={props:{description:{type:String,required:false,default:""}},mounted(){const t=new c.Loader({target:this.$refs.loader,size:65,color:"var(--ui-color-primary)",strokeWidth:4,mode:"inline"});t.show()},computed:{popupOptions(){return{autoHide:false,closeIcon:false}}},components:{SaveProgressPopup:y},template:`\n\t\t<SaveProgressPopup\n\t\t\t:title="$Bitrix.Loc.getMessage('DATASET_IMPORT_PROGRESS_POPUP_HEADER')"\n\t\t\t:description="description"\n\t\t\t:options="popupOptions"\n\t\t>\n\t\t\t<template v-slot:icon>\n\t\t\t\t<div ref="loader" class="dataset-save-progress-loader"></div>\n\t\t\t</template>\n\t\t</SaveProgressPopup>\n\t`};const w={emits:["click","oneMoreClick"],props:{title:{type:String,required:true},description:{type:String,required:false,default:""},datasetId:{type:Number,required:true},showMoreButton:{type:Boolean,required:false,default:false}},computed:{},methods:{onButtonClick(){v.Dom.addClass(this.$refs.openDatasetButton,"ui-btn-wait");v.ajax.runAction("biconnector.externalsource.dataset.getEditUrl",{data:{id:this.datasetId}}).then((t=>{const e=t.data;if(e){window.open(e,"_blank").focus();v.Dom.removeClass(this.$refs.openDatasetButton,"ui-btn-wait");this.$emit("click")}})).catch((t=>{v.Dom.removeClass(this.$refs.openDatasetButton,"ui-btn-wait");this.$emit("click")}))},onOneMoreButtonClick(){v.Dom.addClass(this.$refs.oneMoreButton,"ui-btn-wait");this.$emit("oneMoreClick")}},components:{SaveProgressPopup:y},template:`\n\t\t<SaveProgressPopup\n\t\t\t:title="title"\n\t\t\t:description="description"\n\t\t>\n\t\t\t<template v-slot:icon>\n\t\t\t\t<div class="dataset-save-progress-popup__success-logo"></div>\n\t\t\t</template>\n\t\t\t<template v-slot:buttons>\n\t\t\t\t<a class="ui-btn ui-btn-md ui-btn-primary" @click="onButtonClick" ref="openDatasetButton">\n\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_SUCCESS_POPUP_BUTTON') }}\n\t\t\t\t</a>\n\t\t\t\t<a class="ui-btn ui-btn-md ui-btn-light-border" @click="onOneMoreButtonClick" ref="oneMoreButton" v-if="showMoreButton">\n\t\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_SUCCESS_POPUP_ONE_MORE_BUTTON') }}\n\t\t\t\t</a>\n\t\t\t</template>\n\t\t</SaveProgressPopup>\n\t`};const $={props:{hintClass:{type:String,required:false,default:"ui-alert-primary"}},template:`\n\t\t<div class="ui-alert dataset-import-step__hint" :class="hintClass">\n\t\t\t<span class="ui-alert-message">\n\t\t\t\t<slot></slot>\n\t\t\t</span>\n\t\t</div>\n\t`};const B={data(){return{section:null}},props:{title:{type:String,required:true},hint:{type:String,required:false,default:""},isOpenInitially:{type:Boolean,required:false,default:true},canCollapse:{type:Boolean,required:false,default:true},disabled:{type:Boolean,required:false,default:false},customClasses:{type:Array,default:[]},hintClass:{type:String,required:false}},computed:{additionalClasses(){const t=this.customClasses.reduce(((t,e)=>{t[e]=true;return t}),{});return{"dataset-import-step__disabled":this.disabled,...t}}},methods:{toggleCollapse(t){this.section.toggle(t)}},mounted(){const t=this.$refs.contentContainer;const e=new s.Section({title:this.title,isOpen:this.isOpenInitially,canCollapse:this.canCollapse});e.append(this.$refs.content);e.renderTo(t);this.section=e},watch:{title(t){if(!this.section){return}this.section.getContent().querySelector(".ui-section__title").innerHTML=v.Text.encode(t)}},components:{StepHint:$},template:`\n\t\t<div class="dataset-import-step" :class="additionalClasses" ref="contentContainer">\n\t\t</div>\n\t\t<div ref="content" class="dataset-import-step__content">\n\t\t\t<StepHint v-if="hint" :hint-class="hintClass">\n\t\t\t\t<span v-html="hint"></span>\n\t\t\t</StepHint>\n\t\t\t<slot></slot>\n\t\t</div>\n\t`};const F={emits:["validation"],props:{title:{type:String,required:false},hint:{type:String,required:false},isOpenInitially:{type:Boolean,required:false,default:true},disabled:{type:Boolean,required:false,default:false}},computed:{displayedTitle(){var t;return(t=this.title)!=null?t:this.defaultTitle},displayedHint(){var t;return(t=this.hint)!=null?t:this.defaultHint},defaultTitle(){return""},defaultHint(){return""}},methods:{open(){if(this.$refs.stepBlock){this.$refs.stepBlock.toggleCollapse(true)}},close(){if(this.$refs.stepBlock){this.$refs.stepBlock.toggleCollapse(false)}},validate(){return true},showValidationErrors(){}},components:{Step:B}};let x=t=>t,k;class N{static create(t,e){const i=v.Tag.render(k||(k=x`<span class='ui-hint-content'></span>`));i.innerHTML=t;const s={bindElement:e,darkMode:true,content:i,autoHide:false,bindOptions:{position:"top"},angle:{position:"bottom"},cacheable:false};u.Popup.setOptions({angleMinBottom:43});return new u.Popup(s)}}const V={props:{defaultValue:{required:false,default:""},name:{type:String,required:true},title:{type:String,required:true},placeholder:{type:String,required:false,default:""},isValid:{type:Boolean,required:false,default:true},errorMessage:{type:String,required:false,default:""},isDisabled:{type:Boolean,required:false,default:false},hintText:{type:String,required:false}},emits:["valueChange"],data(){return{value:this.defaultValue,areValidationErrorsShown:false}},methods:{showValidationErrors(){this.areValidationErrorsShown=true},onInputChange(t){this.$emit("valueChange",{newValue:t,fieldName:this.name})}},watch:{defaultValue(t){this.value=t}}};const H={extends:V,data(){return{errorPopup:null,errorPopupTimeout:null}},methods:{onInput(t){this.areValidationErrorsShown=false;this.onInputChange(t.target.value)},onBlur(){this.areValidationErrorsShown=true;if(!this.isValid){this.showErrorHint();this.errorPopupTimeout=setTimeout((()=>{this.hideErrorHint();this.errorPopupTimeout=null}),3e3)}},showErrorHint(){if(this.errorPopupTimeout){clearTimeout(this.errorPopupTimeout);this.errorPopupTimeout=null}else{this.errorPopup=this.createErrorPopup();this.errorPopup.show()}},hideErrorHint(){if(this.errorPopup){this.errorPopup.close()}},createErrorPopup(){return N.create(this.errorMessage,this.$refs.errorIconWrapper)}},mounted(){if(this.hintText){v.Dom.append(BX.UI.Hint.createNode(this.hintText),this.$refs.title)}},template:`\n\t\t<div class="ui-form-row">\n\t\t\t<div class="ui-form-label">\n\t\t\t\t<div class="ui-ctl-label-text" ref="title">\n\t\t\t\t\t{{ title }}\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div class="ui-ctl ui-ctl-after-icon ui-ctl-textbox ui-ctl-w100 dataset-import-control">\n\t\t\t\t<input\n\t\t\t\t\tclass="ui-ctl-element dataset-import-field"\n\t\t\t\t\ttype="text"\n\t\t\t\t\t:class="{ 'data-import-field--invalid': !isValid && areValidationErrorsShown }"\n\t\t\t\t\t:disabled="isDisabled"\n\t\t\t\t\t:placeholder="placeholder"\n\t\t\t\t\tv-model="value"\n\t\t\t\t\t@input="onInput"\n\t\t\t\t\t@blur="onBlur"\n\t\t\t\t>\n\t\t\t\t<div class="ui-ctl-after" ref="errorIconWrapper">\n\t\t\t\t\t<div\n\t\t\t\t\t\tclass="ui-icon-set --warning format-table__error-icon"\n\t\t\t\t\t\t@mouseenter="showErrorHint"\n\t\t\t\t\t\t@mouseleave="hideErrorHint"\n\t\t\t\t\t\tv-if="!isValid && areValidationErrorsShown"\n\t\t\t\t\t></div>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</div>\n\t`};const U={extends:V,template:`\n\t\t<div class="ui-form-row">\n\t\t\t<div class="ui-form-label">\n\t\t\t\t<div class="ui-ctl-label-text">{{ title }}</div>\n\t\t\t</div>\n\t\t\t<div class="ui-ctl ui-ctl-textarea dataset-import-textarea ui-ctl-w100 ui-ctl-no-resize">\n\t\t\t\t<textarea \n\t\t\t\t\tclass="ui-ctl-element dataset-import-textarea-element" \n\t\t\t\t\t:placeholder="placeholder" \n\t\t\t\t\t@input="onInputChange($event.target.value)"\n\t\t\t\t\tv-model="value"\n\t\t\t\t></textarea>\n\t\t\t</div>\n\t\t</div>\n\t`};const q={props:{defaultName:{type:String,required:false,default:""},defaultDescription:{type:String,required:false,default:""},unvalidatedFields:{type:Object,required:false,default:{}},disabledFields:{type:Object,required:false,default:{}},datasetSourceCode:{type:String,required:true}},emits:["valueChange","validationNeeded"],methods:{onValueChange(t){this.$emit("valueChange",t)},showValidationErrors(){this.$refs.nameField.showValidationErrors()}},components:{StringField:H,TextField:U},template:`\n\t\t<div class="ui-form dataset-properties">\n\t\t\t<StringField\n\t\t\t\tref="nameField"\n\t\t\t\tname="name"\n\t\t\t\t:defaultValue="defaultName"\n\t\t\t\t@value-change="onValueChange"\n\t\t\t\t:title="this.$Bitrix.Loc.getMessage('DATASET_IMPORT_DATASET_PROPERTIES_CODE')"\n\t\t\t\t:placeholder="this.$Bitrix.Loc.getMessage('DATASET_IMPORT_DATASET_PROPERTIES_CODE_PLACEHOLDER', { '#CODE#': this.datasetSourceCode })"\n\t\t\t\t:is-valid="unvalidatedFields.name?.result ?? true"\n\t\t\t\t:is-disabled="disabledFields.name ?? false"\n\t\t\t\t:error-message="unvalidatedFields.name?.message ?? ''"\n\t\t\t/>\n\t\t\t<TextField\n\t\t\t\tname="description"\n\t\t\t\t:defaultValue="defaultDescription"\n\t\t\t\t@value-change="onValueChange"\n\t\t\t\t:title="this.$Bitrix.Loc.getMessage('DATASET_IMPORT_DATASET_PROPERTIES_DESCRIPTION')"\n\t\t\t/>\n\t\t</div>\n\t`};const X={extends:F,props:{datasetSourceCode:{type:String,required:true},reservedNames:{type:Array,required:false,default:[]}},emits:["propertiesChanged"],computed:{datasetProperties(){return this.$store.state.config.datasetProperties},defaultTitle(){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_PROPERTIES_HEADER")},defaultHint(){if(this.isEditMode){return""}return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_PROPERTIES_HINT")},disabledFields(){return{name:this.$store.getters.isEditMode}},unvalidatedFields(){const t={};const e=this.validateName();if(!e.result){t.name=e}return t},isEditMode(){return this.$store.getters.isEditMode}},components:{Step:B,DatasetProperties:q},methods:{onDatasetPropertiesFieldsChange(t){const e=this.datasetProperties;e[t.fieldName]=t.newValue;this.$store.commit("setDatasetProperties",e);this.validate();this.$emit("propertiesChanged")},validate(){let t=true;if(!this.isEditMode){t=Object.keys(this.unvalidatedFields).length===0}this.$emit("validation",t);return t},validateName(){const t=this.$store.state.config.datasetProperties.name;if(!t){return{result:true}}const e=this.reservedNames.includes(t);if(e){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_DATASET_EXISTS")}}if(t.length>30){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_DATASET_TOO_LONG")}}if(!/^[a-z][\d_a-z]*$/.test(t)){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_DATASET_INVALID_CHARS")}}return{result:true}},showValidationErrors(){this.$refs.datasetProperties.showValidationErrors()}},template:`\n\t\t<Step\n\t\t\t:title="displayedTitle"\n\t\t\t:hint="displayedHint"\n\t\t\t:is-open-initially="isOpenInitially"\n\t\t\t:disabled="disabled"\n\t\t\tref="stepBlock"\n\t\t>\n\t\t\t<slot name="additional-properties"></slot>\n\t\t\t<DatasetProperties\n\t\t\t\t@value-change="onDatasetPropertiesFieldsChange"\n\t\t\t\t:default-name="datasetProperties.name"\n\t\t\t\t:default-description="datasetProperties.description"\n\t\t\t\tref="datasetProperties"\n\t\t\t\t:disabled-fields="disabledFields"\n\t\t\t\t:unvalidated-fields="unvalidatedFields"\n\t\t\t\t:dataset-source-code="datasetSourceCode"\n\t\t\t/>\n\t\t</Step>\n\t`};const G={props:{enabled:{type:Boolean,required:false,default:true},indeterminate:{type:Boolean,required:false,default:false}},emits:["checkboxClick"],computed:{isNeedShowOriginalNameHint(){var t,e;return(t=(e=this.$store.state.config.fileProperties)==null?void 0:e.firstLineHeader)!=null?t:true}},methods:{onCheckboxClick(t){t.preventDefault();this.$emit("checkboxClick")}},template:`\n\t\t<tr>\n\t\t\t<th class="format-table__header format-table__checkbox-header">\n\t\t\t\t<input class="format-table__checkbox" type="checkbox" @change="onCheckboxClick" :checked="enabled" :indeterminate.prop="indeterminate">\n\t\t\t</th>\n\t\t\t<th class="format-table__header format-table__type-header format-table__type-subfield-header">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_FIELD_SETTINGS_TYPE_HEADER') }}</th>\n\t\t\t<th class="format-table__header format-table__header format-table__title-header">{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_FIELD_SETTINGS_CODE_HEADER') }}</th>\n\t\t\t<th class="format-table__header format-table__header format-table__title-header" v-if="isNeedShowOriginalNameHint"></th>\n\t\t</tr>\n\t`};const j={string:"string",money:"money",int:"int",double:"double",date:"date",datetime:"datetime"};const W={[j.string]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_TEXT"),icon:"--formatting"},[j.money]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_MONEY"),icon:"--money"},[j.int]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_NUMBER"),icon:"--numbers-123"},[j.double]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_DECIMAL"),icon:"--numbers-05"},[j.date]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_DATE"),icon:"--calendar-1"},[j.datetime]:{title:v.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_TYPE_DATETIME"),icon:"--planning-2"}};var Y=babelHelpers.classPrivateFieldLooseKey("selectedType");var K=babelHelpers.classPrivateFieldLooseKey("bindElement");var z=babelHelpers.classPrivateFieldLooseKey("onSelect");var Z=babelHelpers.classPrivateFieldLooseKey("onClose");class Q{constructor(t){Object.defineProperty(this,Y,{writable:true,value:void 0});Object.defineProperty(this,K,{writable:true,value:void 0});Object.defineProperty(this,z,{writable:true,value:void 0});Object.defineProperty(this,Z,{writable:true,value:void 0});babelHelpers.classPrivateFieldLooseBase(this,Y)[Y]=t.selectedType;babelHelpers.classPrivateFieldLooseBase(this,K)[K]=t.bindElement;babelHelpers.classPrivateFieldLooseBase(this,z)[z]=t.onSelect;babelHelpers.classPrivateFieldLooseBase(this,Z)[Z]=t.onClose}getMenu(){const t=[];Object.entries(W).forEach((([e,i])=>{t.push({html:`\n\t\t\t\t\t<div class="ui-icon-set ${i.icon}"></div>\n\t\t\t\t\t<span class="format-table__dropdown-item-text">${i.title}</span>\n\t\t\t\t\t${e===babelHelpers.classPrivateFieldLooseBase(this,Y)[Y]?'<div class="format-table__dropdown-item-selected ui-icon-set --check"></div>':""}\n\t\t\t\t`,onclick:(t,i)=>{babelHelpers.classPrivateFieldLooseBase(this,z)[z](e);i.getMenuWindow().close()},className:`format-table__dropdown-item${e===babelHelpers.classPrivateFieldLooseBase(this,Y)[Y]?" format-table__dropdown-item--active":""}`})}));return new u.Menu({className:"format-table__dropdown-popup",bindElement:babelHelpers.classPrivateFieldLooseBase(this,K)[K],items:t,events:{onClose:babelHelpers.classPrivateFieldLooseBase(this,Z)[Z]}})}}const J={props:{selectedType:{type:String,required:false,default:"text"},canEdit:{type:Boolean,required:false,default:false}},emits:["valueChange"],data(){return{typeMenu:null,isFocused:false}},computed:{iconClass(){return W[this.selectedType].icon},typeTitle(){return W[this.selectedType].title},isEditable(){return this.canEdit}},methods:{onClick(t){if(!this.isEditable){return}this.isFocused=true;if(!this.typeMenu){return}this.typeMenu.getPopupWindow().setWidth(this.$refs.formatButton.offsetWidth);this.typeMenu.show()},createMenu(){if(this.isEditable){this.typeMenu=new Q({selectedType:this.selectedType,bindElement:this.$refs.formatButton,onSelect:t=>{this.$emit("valueChange",t)},onClose:()=>{this.isFocused=false}}).getMenu()}},destroyMenu(){var t;(t=this.typeMenu)==null?void 0:t.destroy()}},mounted(){this.createMenu()},beforeUnmount(){this.destroyMenu()},watch:{selectedType(t){this.destroyMenu();this.createMenu()},isEditable(t){if(t&&!this.typeMenu){this.createMenu()}if(!t){this.destroyMenu()}}},template:`\n\t\t<div\n\t\t\tclass="ui-ctl ui-ctl-before-icon ui-ctl-after-icon ui-ctl-w100 format-table__type-control ui-ctl-dropdown"\n\t\t\tref="formatButton"\n\t\t\t@click="onClick"\n\t\t\t:class="{\n\t\t\t\t'format-table__type-control--disabled': !isEditable,\n\t\t\t\t'ui-ctl-hover': isFocused,\n\t\t\t}"\n\t\t>\n\t\t\t<div class="ui-ctl-after ui-ctl-icon-angle" v-if="isEditable"></div>\n\t\t\t<div class="ui-ctl-before">\n\t\t\t\t<div class="format-table__type-button" :class="{ 'format-table__type-button--disabled': !isEditable }">\n\t\t\t\t\t<div class="ui-icon-set" :class="iconClass"></div>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<div class="ui-ctl-element format-table__text-input">\n\t\t\t\t<span>{{ typeTitle }}</span>\n\t\t\t</div>\n\t\t</div>\n\t`};const tt={directives:{hint:T.hint},props:{enabled:{type:Boolean,required:false,default:true},index:{type:Number,required:true},fieldSettings:{type:Object,required:false,default:null},invalidFields:{type:Object,required:false,default:{}}},data(){return{displayedValidationErrors:{name:true},errorPopup:{name:null},errorPopupTimeout:null}},computed:{isNameValid(){return!("name"in this.invalidFields)},displayedErrorForName(){return this.invalidFields.name.message},set(){return _.Set},originalsHintText(){const t=this.fieldSettings.originalType;const e=this.fieldSettings.originalName;let i="";if(t){i=this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_ORIG_TYPE",{"#CLASS#":'<span class="format-table__orig_info_title">',"#/CLASS#":"</span>","#TYPENAME#":W[t].title})}const s=this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_SETTINGS_ORIG_NAME",{"#CLASS#":'<span class="format-table__orig_info_title">',"#/CLASS#":"</span>","#FIELDNAME#":e});if(!t){return s}return`${i}<br>${s}`},isNeedShowOriginalNameHint(){var t,e,i;return((t=(e=this.$store.state.config.fileProperties)==null?void 0:e.firstLineHeader)!=null?t:true)&&((i=this.$store.state.config.connectionProperties)==null?void 0:i.connectionType)},hintOptions(){return{html:this.originalsHintText,popupOptions:{angle:{position:"left"},offsetLeft:30,offsetTop:-46,autoHide:false}}},canEdit(){var t;return!Boolean(((t=this.fieldSettings)==null?void 0:t.id)>0)}},emits:["checkboxClick","fieldChange"],methods:{onCheckboxClick(t){t.preventDefault();this.$emit("checkboxClick",{index:this.index})},onTypeSelected(t){this.$emit("fieldChange",{fieldName:"type",value:t,index:this.index})},onFieldInput(t){this.displayedValidationErrors[t.target.name]=false;this.$emit("fieldChange",{fieldName:t.target.name,value:t.target.value,index:this.index})},onFieldBlur(t){this.displayedValidationErrors[t.target.name]=true;if("name"in this.invalidFields){this.showErrorHint();this.errorPopupTimeout=setTimeout((()=>{this.hideErrorHint();this.errorPopupTimeout=null}),3e3)}},showValidationErrors(){Object.keys(this.displayedValidationErrors).forEach((t=>{this.displayedValidationErrors[t]=true}))},showErrorHint(){if(this.errorPopupTimeout){clearTimeout(this.errorPopupTimeout);this.errorPopupTimeout=null}else{this.errorPopup.name=this.createErrorPopup();this.errorPopup.name.show()}},hideErrorHint(){if(this.errorPopup.name){this.errorPopup.name.close()}},createErrorPopup(){return N.create(this.displayedErrorForName,this.$refs.errorIconWrapper)}},watch:{isNeedShowOriginalNameHint(){if(this.$refs.originalsHint){this.$refs.originalsHint.remove()}}},components:{DataTypeButton:J,BIcon:_.BIcon},template:`\n\t\t<tr class="format-table__row">\n\t\t\t<td class="format-table__checkbox-cell">\n\t\t\t\t<input class="format-table__checkbox" ref="visibilityCheckbox" type="checkbox" @change="onCheckboxClick" :checked="enabled">\n\t\t\t</td>\n\t\t\t<td class="format-table__cell">\n\t\t\t\t<DataTypeButton :selected-type="fieldSettings.type" @value-change="onTypeSelected" :can-edit="canEdit" />\n\t\t\t</td>\n\t\t\t<td class="format-table__cell">\n\t\t\t\t<div\n\t\t\t\t\tclass="ui-ctl ui-ctl-textbox ui-ctl-w100 format-table__name-control"\n\t\t\t\t\t:class="{\n\t\t\t\t\t\t'format-table__text-input--invalid': displayedValidationErrors.name && !isNameValid,\n\t\t\t\t\t\t'format-table__text-input--disabled': !canEdit,\n\t\t\t\t\t\t'ui-ctl-after-icon': canEdit && !isNameValid,\n\t\t\t\t\t}"\n\t\t\t\t>\n\t\t\t\t\t<input\n\t\t\t\t\t\tclass="ui-ctl-element format-table__text-input format-table__name-input"\n\t\t\t\t\t\t:disabled="!canEdit"\n\t\t\t\t\t\ttype="text"\n\t\t\t\t\t\t:placeholder="$Bitrix.Loc.getMessage('DATASET_IMPORT_FIELD_SETTINGS_PLACEHOLDER')"\n\t\t\t\t\t\tname="name"\n\t\t\t\t\t\t@input="onFieldInput"\n\t\t\t\t\t\t@blur="onFieldBlur"\n\t\t\t\t\t\t:value="fieldSettings.name"\n\t\t\t\t\t>\n\t\t\t\t\t<div class="ui-ctl-after" ref="errorIconWrapper">\n\t\t\t\t\t\t<div\n\t\t\t\t\t\t\tclass="ui-icon-set --warning format-table__error-icon"\n\t\t\t\t\t\t\t@mouseenter="showErrorHint"\n\t\t\t\t\t\t\t@mouseleave="hideErrorHint"\n\t\t\t\t\t\t\tv-if="displayedValidationErrors.name && !isNameValid"\n\t\t\t\t\t\t></div>\n\t\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t</td>\n\t\t\t<td class="format-table__cell" v-if="isNeedShowOriginalNameHint">\n\t\t\t\t<div class="format-table__orig-type-hint-wrapper" v-hint="hintOptions" ref="originalsHint">\n\t\t\t\t\t<div class="format-table__orig-type-hint">\n\t\t\t\t\t\t<BIcon\n\t\t\t\t\t\t\t:name="set.INFO_1"\n\t\t\t\t\t\t\t:size="20"\n\t\t\t\t\t\t\tcolor="#B5BABE"\n\t\t\t\t\t\t></BIcon>\n\t\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t</td>\n\t\t</tr>\n\t`};const et={props:{fieldsSettings:{type:Array,required:false},unvalidatedRows:{type:Object,required:false}},emits:["rowToggle","headerToggle","rowFieldChanged"],computed:{areAllRowsVisible(){return this.$store.getters.areAllRowsVisible},areNoRowsVisible(){return this.$store.getters.areNoRowsVisible},areSomeRowsVisible(){return this.$store.getters.areSomeRowsVisible}},methods:{onRowCheckboxClicked(t){this.$emit("rowToggle",t)},onHeaderCheckboxClicked(){this.$emit("headerToggle")},onRowFieldChanged(t){this.$emit("rowFieldChanged",t)},showValidationErrors(){const t=Object.keys(this.unvalidatedRows);t.forEach((t=>{if(this.$refs.row[t]){this.$refs.row[t].showValidationErrors()}}))}},components:{TableHeader:G,TableRow:tt},template:`\n\t\t<table class="format-table">\n\t\t\t<thead>\n\t\t\t\t<TableHeader :enabled="areAllRowsVisible" :indeterminate="areSomeRowsVisible" @checkbox-click="onHeaderCheckboxClicked" />\n\t\t\t</thead>\n\t\t\t<tbody>\n\t\t\t\t<template v-for="(field, index) in fieldsSettings" :key="index">\n\t\t\t\t\t<TableRow\n\t\t\t\t\t\tref="row"\n\t\t\t\t\t\t:index="index"\n\t\t\t\t\t\t:enabled="field.visible"\n\t\t\t\t\t\t:field-settings="field"\n\t\t\t\t\t\t@checkbox-click="onRowCheckboxClicked"\n\t\t\t\t\t\t@field-change="onRowFieldChanged"\n\t\t\t\t\t\t:invalid-fields="unvalidatedRows[index] ?? []"\n\t\t\t\t\t/>\n\t\t\t\t</template>\n\t\t\t</tbody>\n\t\t</table>\n\t`};const it={emits:["buttonClick"],props:{disabled:{type:Boolean,default:false}},computed:{isDisabled(){return this.disabled}},methods:{onButtonClick(){this.$emit("buttonClick")}},template:`\n\t\t<div class="ui-form-row">\n\t\t\t<button :disabled="isDisabled" class="ui-btn ui-btn-sm ui-btn-primary ui-btn-no-caps" @click="onButtonClick">\n\t\t\t\t{{ $Bitrix.Loc.getMessage('DATASET_IMPORT_SYNC_FIELDS_BUTTON') }}\n\t\t\t</button>\n\t\t</div>\n\t`};const st={emits:["parsingOptionsChanged","settingsChanged","syncFields"],extends:F,props:{syncFieldsProps:{type:Object,default:{supported:false,disabled:true}}},computed:{fieldsSettings(){return this.$store.state.config.fieldsSettings},defaultTitle(){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELDS_SETTINGS_HEADER")},defaultHint(){if(this.isEditMode){return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELDS_SETTINGS_HINT_EDIT").replace("[link]",'<a onclick="top.BX.Helper.show(`redirect=detail&code=23378698`)">').replace("[/link]","</a>")}return this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELDS_SETTINGS_HINT").replace("[link]",'<a onclick="top.BX.Helper.show(`redirect=detail&code=23378698`)">').replace("[/link]","</a>")},unvalidatedRows(){const t={};this.$store.state.config.fieldsSettings.forEach(((e,i)=>{const s={};if(!e.id){const t=this.validateName(e.name);if(!t.result){s.name=t}}if(Object.keys(s).length>0){t[i]=s}}));return t},isEditMode(){return this.$store.getters.isEditMode},isSyncSupported(){return this.syncFieldsProps.supported},isSyncDisabled(){return this.syncFieldsProps.disabled}},methods:{validateName(t){const e=t.length>0;if(!e){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_EMPTY_ERROR")}}const i=t.length>32;if(i){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_FIELD_TOO_LONG")}}const s=/^[A-Z](?=.*[A-Z_])[A-Z0-9_]*$/.test(t);if(!s){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_FIELD_INVALID_CHARS")}}const n=this.$store.getters.previewHeaders.reduce(((e,i)=>t===i?e+1:e),0)>1;if(n){return{result:false,message:this.$Bitrix.Loc.getMessage("DATASET_IMPORT_FIELD_VALIDATION_FIELD_ALREADY_USED")}}return{result:true}},onRowToggled(t){this.$store.commit("toggleRowVisibility",t.index);this.$emit("settingsChanged")},onHeaderToggled(t){if(this.$store.getters.areAllRowsVisible){this.$store.commit("setAllRowsInvisible")}else{this.$store.commit("setAllRowsVisible")}this.$emit("settingsChanged")},onRowFieldChanged(t){this.fieldsSettings[t.index][t.fieldName]=t.value;this.$store.commit("setFieldRowSettings",{index:t.index,settings:this.fieldsSettings[t.index]});if(t.fieldName==="type"){this.$emit("parsingOptionsChanged")}this.$emit("settingsChanged");this.validate()},validate(){const t=Object.keys(this.unvalidatedRows).length===0;this.$emit("validation",t);return t},showValidationErrors(){this.$refs.formatTable.showValidationErrors()}},components:{Step:B,FormatTable:et,SyncFieldsButton:it},template:`\n\t\t<Step\n\t\t\t:title="displayedTitle"\n\t\t\t:hint="displayedHint"\n\t\t\t:is-open-initially="isOpenInitially"\n\t\t\t:disabled="disabled"\n\t\t\tref="stepBlock"\n\t\t>\n\t\t\t<SyncFieldsButton v-if="isSyncSupported" :disabled="isSyncDisabled" @button-click="this.$emit('syncFields')"/>\n\t\t\t<div class="ui-form-row fields-settings">\n\t\t\t\t<FormatTable\n\t\t\t\t\t:fields-settings="fieldsSettings"\n\t\t\t\t\t@row-toggle="onRowToggled"\n\t\t\t\t\t@header-toggle="onHeaderToggled"\n\t\t\t\t\t@row-field-changed="onRowFieldChanged"\n\t\t\t\t\t:unvalidated-rows="unvalidatedRows"\n\t\t\t\t\tref="formatTable"\n\t\t\t\t/>\n\t\t\t</div>\n\t\t</Step>\n\t`};let nt=t=>t,ot,at,rt,lt,dt,ct,pt;const ut={CUSTOM:"custom",VALUE:"value"};var ht=babelHelpers.classPrivateFieldLooseKey("extractValues");var gt=babelHelpers.classPrivateFieldLooseKey("getContent");var mt=babelHelpers.classPrivateFieldLooseKey("getDropdownControl");class ft{static openSlider(t,e,i){BX.SidePanel.Instance.open("biconnector:import-field-formats",{width:584,contentCallback:()=>a.Layout.createContent({extensions:["ui.forms","ui.layout-form","ui.alerts","biconnector.dataset-import"],title:v.Loc.getMessage("DATASET_IMPORT_FIELD_FORMAT_SELECTOR_TITLE"),content(){return babelHelpers.classPrivateFieldLooseBase(ft,gt)[gt](t,e)},buttons({cancelButton:t,SaveButton:e}){return[new e({onclick:()=>{const t=BX.SidePanel.Instance.getTopSlider().getContainer().querySelector("#formatSelectorForm");i(babelHelpers.classPrivateFieldLooseBase(ft,ht)[ht](t));BX.SidePanel.Instance.close()}}),t]}}),cacheable:false})}}function _t(t){const e=new FormData(t);const i=Object.fromEntries(e);const s=["date","datetime"];s.forEach((t=>{if(i[t]==="custom"){i[t]=i[`${t}CustomValue`]}delete i[`${t}CustomValue`]}));return i}function Tt(t,e){const i=v.Tag.render(ot||(ot=nt`
		...
		`),v.Loc.getMessage("DATASET_IMPORT_FIELD_FORMAT_SELECTOR_HINT",{"[link]":'<a onclick="top.BX.Helper.show(`redirect=detail&code=23378698`)">',"[/link]":"</a>"}),i)}function Et(t){const e=v.Tag.render(rt||(rt=nt`
			<select class="ui-ctl-element" name="${0}">
			</select>
		`),t.fieldType);let i=true;let s=null;let n=null;t.options.forEach((o=>{let a="";const r=o.value===t.selected;if(r){i=false}if(o.type===ut.VALUE){a=v.Tag.render(lt||(lt=nt`
					<option ${0} value="${0}">${0}</option>
				`),o.value===t.selected?"selected":"",o.value,o.title)}else if(o.type===ut.CUSTOM){var l;s=v.Tag.render(dt||(dt=nt`
					<option value="custom">${0}</option>
				`),v.Loc.getMessage("DATASET_IMPORT_FIELD_FORMAT_SELECTOR_CUSTOM_FORMAT"));a=s;n=v.Tag.render(ct||(ct=nt`
				...
				`),t.fieldType,(l=o.value)!=null?l:"");v.Event.bind(e,"change",(t=>{const e=t.target.value;if(e==="custom"){v.Dom.removeClass(n,"format-selector__custom-value-input--hidden")}else{v.Dom.addClass(n,"format-selector__custom-value-input--hidden")}}))}v.Dom.append(a,e)}));if(i){if(s){s.setAttribute("selected",true)}if(n){n.querySelector("input").value=t.selected;v.Dom.removeClass(n,"format-selector__custom-value-input--hidden")}}return v.Tag.render(pt||(pt=nt`
			<div class="ui-form-row">
				<div class="ui-form-label">
