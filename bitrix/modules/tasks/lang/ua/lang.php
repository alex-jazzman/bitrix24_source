<?php
$MESS["TASKS_ACCESS_DENIED"] = "Недостатньо прав для вчинення дії";
$MESS["TASKS_ACTION_NOT_ALLOWED"] = "Дія над завданням не дозволена.";
$MESS["TASKS_BAD_ASSIGNEE"] = "Виконавець не зазначений.";
$MESS["TASKS_BAD_ASSIGNEE_EX"] = "Користувач зазначений у полі «Виконавець» не знайдений.";
$MESS["TASKS_BAD_CREATED_BY"] = "Не вказаний постановник";
$MESS["TASKS_BAD_DEPENDS_ON_ID"] = "Не вказано попереднє завдання";
$MESS["TASKS_BAD_DEPENDS_ON_ID_EX"] = "Попереднє завдання не знайдено";
$MESS["TASKS_BAD_DURATION"] = "У плануванні термінів вказана занадто велика тривалість завдання";
$MESS["TASKS_BAD_FILE_ID"] = "Не вказано файл";
$MESS["TASKS_BAD_FILE_ID_EX"] = "Файл не знайдено";
$MESS["TASKS_BAD_NAME"] = "Не зазначений тег";
$MESS["TASKS_BAD_PARENT_ID"] = "Завдання, зазначене в полі «Надзавдання», не знайдено";
$MESS["TASKS_BAD_PLAN_DATES"] = "У плануванні термінів вказана дата закінчення менша дати старту";
$MESS["TASKS_BAD_REMIND_DATE"] = "Не вказана дата нагадування";
$MESS["TASKS_BAD_RESPONSIBLE_ID"] = "Не зазначений відповідальний";
$MESS["TASKS_BAD_RESPONSIBLE_ID_EX"] = "Користувач, зазначений у полі «Відповідальний», не знайдений";
$MESS["TASKS_BAD_TASK_ID"] = "Не вказано завдання";
$MESS["TASKS_BAD_TASK_ID_EX"] = "Завдання не знайдене";
$MESS["TASKS_BAD_TITLE"] = "Не зазначена назва завдання";
$MESS["TASKS_BAD_USER_ID"] = "Не вказаний користувач";
$MESS["TASKS_BAD_USER_ID_EX"] = "Користувач не знайдений";
$MESS["TASKS_CHECKLISTITEM_BAD_COMPLETE_FLAG"] = "Некоректно вказаний прапор завершеності";
$MESS["TASKS_CHECKLISTITEM_BAD_SORT_DIRECTION"] = "Некоректний напрямок сортування в масиві сортування";
$MESS["TASKS_CHECKLISTITEM_BAD_TITLE"] = "Не вказано назву";
$MESS["TASKS_CHECKLISTITEM_UNKNOWN_FIELD"] = "Невідоме поле в масиві-аргументі";
$MESS["TASKS_DEPENDS_ON_SELF"] = "Завдання не може залежати само від себе";
$MESS["TASKS_DESCRIPTION_IN_BBCODE_NO_NOT_ALLOWED"] = "Створення нових завдань з описом в HTML форматі заборонено";
$MESS["TASKS_ERR_GROUP_IN_USE"] = "Група не може бути вилучена, тому що до неї прив'язані завдання";
$MESS["TASKS_ERR_USER_IN_USE_TAIL"] = ", ... (ще #N#)";
$MESS["TASKS_ERR_USER_IN_USE_TASKS"] = "завдання: #IDS#";
$MESS["TASKS_ERR_USER_IN_USE_TASKS_PREFIX"] = "Користувач не може бути видалений, так як є учасником наступних #ENTITIES#";
$MESS["TASKS_ERR_USER_IN_USE_TEMPLATES"] = "шаблони завдань: #IDS#";
$MESS["TASKS_FAILED_TO_DO_ACTION"] = "Не вдалося виконати дію";
$MESS["TASKS_INCORRECT_STATUS"] = "Некоректний статус";
$MESS["TASKS_IS_LINKED_END_DATE_PLAN_REMOVE"] = "Неможливо вилучити час завершення завдання: завдання має залежності";
$MESS["TASKS_IS_LINKED_SET_PARENT"] = "Неможливо призначити батьківське завдання: завдання має залежність від даного завдання";
$MESS["TASKS_LONG_TITLE"] = "Занадто довга назва завдання";
$MESS["TASKS_PARENT_SELF"] = "Завдання не може бути підзавданням самого себе";
$MESS["TASKS_RIGHTS_MORE_LINK"] = "Докладніше про права доступу";
$MESS["TASKS_TASK_CREATE_ACCESS_DENIED"] = "Недостатньо прав для створення завдання";
$MESS["TASKS_TEMPLATE_CREATE_TASK_NOT_ACCESSIBLE"] = "Недостатньо прав для створення завдань за шаблоном з вибраними учасниками (постановник повинен мати право на створення завдань виконавцям, а також мати доступ до проєкту завдання). Призначте відповідні дозволи й повторіть спробу.";
$MESS["TASKS_TEMPLATE_CREATE_TASK_NOT_ACCESSIBLE_MSGVER_1"] = "Недостатньо прав для створення завдань за шаблоном із вибраними учасниками (постановник має мати право на створення завдань виконавцям / також у межах проєкту). #BR# #LINK_START#Налаштуйте права доступу#LINK_END# та повторіть спробу збереження шаблону або виберіть інших учасників.";
$MESS["TASKS_TEMPLATE_MEMBER_CONVERT"] = "Зараз ми оновлюємо шаблони на більш зручні. Оновлення не займе багато часу, ви можете продовжити роботу в інших розділах Бітрікс24 й повернутися пізніше.";
$MESS["TASKS_TEMPLATE_MEMBER_CONVERT_TITLE"] = "Зачекайте трохи, #BR#шаблони скоро з’являться.";
$MESS["TASKS_UNKNOWN_ADD_ERROR"] = "Помилка додавання завдання";
$MESS["TASKS_UNKNOWN_UPDATE_ERROR"] = "Помилка оновлення завдання";
