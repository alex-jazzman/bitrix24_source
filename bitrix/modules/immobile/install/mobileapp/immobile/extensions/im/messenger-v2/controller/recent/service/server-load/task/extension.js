/**
 * @module im/messenger-v2/controller/recent/service/server-load/task
 */
jn.define('im/messenger-v2/controller/recent/service/server-load/task', (require, exports, module) => {
	 ...
	 * @class TaskServerLoadService
	 */
	class TaskServerLoadService extends BaseRecentService
	{
		onInit()
		{
		...
		/**
		 * @param {immobileTabTaskLoadResultTaskList|imV2TaskTailResult} result
		 */
		...
		/**
		 * @param {imV2TaskTailResult} result
		 */
		...
		async createRequest(restOptions)
		{
			try
			{
				...
				const resultRequest = await RecentRest.getExternalChatList(restOptions);
		...
		/**
		 * @param {RefreshModeType} mode
		 * @param {immobileTabTaskLoadResultV2} initResult
		 * @return {Promise<void>}
		 */
		async handleInitResult(mode, initResult)
		{
			...
			if (!initResult || !Type.isPlainObject(initResult) || !Type.isArray(initResult.taskList?.recentItems))
			{
			...
			try
			{
				await this.processResult(initResult.taskList, true);
				this.setPaginationState(initResult.taskList);
			}
			catch (error)
			{
		...
		/**
		 * @param {immobileTabTaskLoadResultTaskList} initResultList
		 */
		...
		/**
		 * @param {immobileTabTaskLoadResultTaskList} restResult
		...
		/**
		 * @param {immobileTabTaskLoadResultTaskList} restResult
		 * @return {TaskTailResultMessageData[]}
		 */
	...
	module.exports = TaskServerLoadService;
});
