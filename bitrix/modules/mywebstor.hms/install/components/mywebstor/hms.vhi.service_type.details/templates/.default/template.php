...
$removeButton = new \Bitrix\UI\Buttons\Button(array(
    "text" => Loc::getMessage("HMS_VHI_SERVICE_TYPE_BUTTON_DELETE"),
    "className" => "ui-btn-light-border",
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.MyWebstor.HMS.Vhi.ServiceType.Details.Instance.delete();'
    )
));
if ($arResult["ID"] > 0)
...
<script>
    BX.ready(function() {
        var vhiDetails = BX.MyWebstor.HMS.Vhi.ServiceType.Details.getInstance(
            <?=
