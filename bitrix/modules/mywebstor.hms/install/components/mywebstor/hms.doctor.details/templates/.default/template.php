 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \LicensesLicenseDetailsComponent $component
...
$removeButton = new \Bitrix\UI\Buttons\Button(array(
    "text" => Loc::getMessage("HMS_DOCTOR_BUTTON_DELETE"),
    "className" => "ui-btn-light-border",
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.MyWebstor.HMS.Doctor.Details.Instance.delete();'
    )
));

$docgenButton = new \Bitrix\UI\Buttons\Button(array(
    "text" => Loc::getMessage('HMS_DOCTOR_BUTTON_COMMON_DOCUMENT'),
...
<script>
    BX.ready(function() {
        var doctorDetails = BX.MyWebstor.HMS.Doctor.Details.getInstance(
