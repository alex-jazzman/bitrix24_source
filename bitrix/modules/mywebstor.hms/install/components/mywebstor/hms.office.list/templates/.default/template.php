 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsOfficeListComponent $component
...
$addButton = new \Bitrix\UI\Buttons\AddButton(array(
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.SidePanel.Instance.open("/hms/config/office/details/0/");'
    )
));
...
<script>
    BX.ready(function() {
        var clinicList = BX.MyWebstor.HMS.Office.List.getInstance("<?= $arResult["GRID_ID"] ?>");
    });
</script>
