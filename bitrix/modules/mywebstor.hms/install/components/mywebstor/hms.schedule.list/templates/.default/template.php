 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsScheduleListComponent $component
...
$addButton = new \Bitrix\UI\Buttons\AddButton(array(
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.SidePanel.Instance.open("/hms/config/schedule/details/0/");'
    )
));
...
<script>
    BX.ready(function() {
        var scheduleList = BX.MyWebstor.HMS.Schedule.List.getInstance("<?= $arResult["GRID_ID"] ?>");
    });
</script>
