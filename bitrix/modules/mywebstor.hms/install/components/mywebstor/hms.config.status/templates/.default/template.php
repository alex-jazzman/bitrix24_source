...
$initialFieldsArray = $arResult['INITIAL_FIELDS'];
		...
		<? foreach($arResult['HEADERS'] as $entityId => $headerName): ?>
			...
			<div id="content_<?=$entityId?>"
				...
				<div class="transaction-stage transaction-initial-stage">
                    ...
                    <?
                        foreach($initialFieldsArray[$entityId] as $initialFields) {
                            $iconClass = '';
                            $blockClass = '';
                            $colorValue = $initialFields['COLOR'] ? $initialFields['COLOR'] : '#ACE9FB';
                            $style = 'background:'.$colorValue.';';
                            $style .= 'color:'.getColorText($colorValue, $iconClass, $blockClass).';';
					?>
					<div class="transaction-stage-phase" data-sort="<?=$initialFields['SORT']?>"
						 id="field-phase-<?=$initialFields['ID']?>" data-calculate="1" data-success="1"
						 ondblclick="BX['<?=$jsClass?>'].editField('<?=$initialFields['ID']?>');"
						...
						<div id="phase-panel" data-class="transaction-stage-phase-panel"
							...
							<?if($initialFields['SYSTEM'] == 'Y' &&
								!empty($initialFields['NAME_INIT'])):?>
								<div onclick="BX['<?=$jsClass?>'].recoveryName('<?=$initialFields['ID']?>',
									'<?=htmlspecialcharsbx($initialFields['NAME_INIT'])?>')"
						...
						<span id="phase-panel" data-class="transaction-stage-phase-title"
							...
							<span id="field-title-inner-<?=$initialFields['ID']?>" class="transaction-stage-phase-title-inner">
								<span id="field-name-<?=$initialFields['ID']?>" class="transaction-stage-phase-name">
									<?=$initialFields['NUMBER']?>.
									<?=htmlspecialcharsbx($initialFields['NAME'])?>
								</span>
								<span onclick="BX['<?=$jsClass?>'].editField('<?=$initialFields['ID']?>');"
									  title="<?=GetMessage('CRM_STATUS_EDIT_NAME')?>"
									  class="transaction-stage-phase-icon-edit"></span>
							</span>
						</span>

						<input type="hidden" id="field-number-<?=$initialFields['ID']?>"
							   value="<?=$initialFields['NUMBER']?>">
						<input type="hidden" name="LIST[<?=$entityId?>][<?=$initialFields['ID']?>][SORT]"
							   id="field-sort-<?=$initialFields['ID']?>"
							   value="<?=$initialFields['SORT']?>">
						<input type="hidden" name="LIST[<?=$entityId?>][<?=$initialFields['ID']?>][VALUE]"
							   id="field-hidden-name-<?=$initialFields['ID']?>"
							   value="<?=htmlspecialcharsbx($initialFields['NAME'])?>">
						<input type="hidden" name="LIST[<?=$entityId?>][<?=$initialFields['ID']?>][COLOR]"
							   id="stage-color-<?=$initialFields['ID']?>"
							   value="<?=htmlspecialcharsbx($colorValue)?>">
						<input type="hidden" name="LIST[<?=$entityId?>][<?=$initialFields['ID']?>][STATUS_ID]"
							   id="stage-status-id-<?=$initialFields['ID']?>" data-status-id="1"
							   value="<?=htmlspecialcharsbx($initialFields['STATUS_ID'])?>">
					</div>
                    <? } ?>
				</div>
...
<?endforeach;?>
		...
		BX['<?=$jsClass?>'] = new BX.CrmConfigStatusClass({
			...
			ajaxUrl: "<?=SITE_DIR?>bitrix/components/mywebstor/hms.config.status/ajax.php?&<?=bitrix_sessid_get()?>",
			...
			initialFields: <?=CUtil::PhpToJSObject($initialFieldsArray)?>,
