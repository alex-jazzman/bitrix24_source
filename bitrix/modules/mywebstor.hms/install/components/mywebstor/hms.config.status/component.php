...
if (!CModule::IncludeModule('mywebstor.hms')) {
    ShowError(GetMessage('HMS_MODULE_NOT_INSTALLED'));
    return;
}
...
if ($arResult['ACTIVE_TAB'] === '') {
    $arResult['ACTIVE_TAB'] = 'status_tab_HMS_APPOINTMENT_STATUS';
    $arResult['ACTIVE_ENTITY_ID'] = 'HMS_APPOINTMENT_STATUS';
}
    ...
    foreach ($_POST['LIST'] as $entityId => $arFields) {
        ...
        if (array_key_exists('REMOVE', $arFields) && is_array($arFields['REMOVE'])) {
            ...
            if (!empty($listField)) {
                ...
                $error = GetMessage(
                    'CRM_MODULE_ERROR_REMOVE_FIELD' . $langString,
...
foreach (\MyWebstorHmsHelper::getStatusEntityTypes() as $entityId => $arEntityType) {
...
foreach ($arResult['ENTITY'] as $entityId => $dataEntity) {
    ...
    foreach ($arResult['ROWS'][$entityId] as $status) {
        ...
        if ($status["SYSTEM"] == "Y" && $status['SEMANTICS'] === \Bitrix\Crm\PhaseSemantics::PROCESS) {
