 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsFillingMethodListComponent $component
...
$addButton = new \Bitrix\UI\Buttons\AddButton(array(
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.SidePanel.Instance.open("/hms/config/filling_method/details/0/");'
    )
));
...
<script>
    BX.ready(function() {
        var fillingMethodList = BX.MyWebstor.HMS.FillingMethod.List.getInstance("<?= $arResult["GRID_ID"] ?>");
    });
</script>
