 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsVhiDetailsComponent $component
...
$removeButton = new \Bitrix\UI\Buttons\Button(array(
    "text" => Loc::getMessage("HMS_VHI_BUTTON_DELETE"),
    "className" => "ui-btn-light-border",
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.MyWebstor.HMS.Vhi.Details.Instance.delete();'
    )
));
if ($arResult["ID"] > 0)
...
<script>
    BX.ready(function() {
        var vhiDetails = BX.MyWebstor.HMS.Vhi.Details.getInstance(
            <?=
