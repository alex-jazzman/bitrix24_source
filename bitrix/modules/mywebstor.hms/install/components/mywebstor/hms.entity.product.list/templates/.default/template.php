 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsEntityProductListComponent $component
...
$editorConfig = [
    ...
    'reloadUrl' => '/bitrix/components/mywebstor/hms.entity.product.list/list.ajax.php',
...
foreach ($grid['ROWS'] as $product) {
    ...
    $item = [
        ...
        'IS_VHI' => $rawProduct['IS_VHI'],
        'TEETH' => $rawProduct['TEETH'],
    ...
    // region IS_VHI
    $product["IS_VHI"] = $APPLICATION->IncludeComponent(
        "mywebstor:hms.grid.product.field.is_vhi",
        "",
        array(
            "ROW_ID" => $rowId,
            "VALUE" => $rawProduct['IS_VHI'],
        )
    );
    // end region IS_VHI

    // region TEETH
    if ($arParams["ENABLE_TEETH_FIELD"])
        $product['TEETH'] = $APPLICATION->IncludeComponent(
            "mywebstor:hms.grid.product.field.teeth",
            "",
            array(
                "ROW_ID" => $rowId,
                "VALUE" => $rawProduct['TEETH'],
            )
        );
    // end region TEETH
...
<div class="crm-entity-product-list-wrapper" id="<?= $containerId ?>"><?php
        ...
        <div class="<?= $buttonTopPanelClasses ?>">
            <div>
                ...
                ?>
                <a class="ui-btn ui-btn-primary <?= $disabledAddRowButton ? $lockedClasses : '' ?>" data-role="product-list-add-button" title="<?= Loc::getMessage('CRM_ENTITY_PL_ADD_PRODUCT_TITLE') ?>" tabindex="-1" <?= $disabledAddRowButton ? $buttonHintAttributes : '' ?>>
                    <?= Loc::getMessage('CRM_ENTITY_PL_ADD_PRODUCT') ?>
                </a>
                <!-- <a class="ui-btn ui-btn-light-border <?= $disabledSelectProductButton ? $lockedClasses : '' ?>" data-role="product-list-select-button" title="<?= Loc::getMessage('CRM_ENTITY_PL_SELECT_PRODUCT_TITLE') ?>" tabindex="-1" <?= $disabledSelectProductButton ? $buttonHintAttributes : '' ?>>
                    <?= Loc::getMessage('CRM_ENTITY_PL_SELECT_PRODUCT') ?>
                </a> -->
            </div>
            <button class="ui-btn ui-btn-light-border ui-btn-icon-setting" data-role="product-list-settings-button"></button>
        </div>
    <?php
                                                                        }
