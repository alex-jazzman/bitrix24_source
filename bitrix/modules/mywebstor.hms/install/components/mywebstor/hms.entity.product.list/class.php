...
final class HmsEntityProductListComponent
...
implements Main\Engine\Contract\Controllerable, Main\Errorable {
    ...
    protected function prepareEntitySettings(array &$params): void {
        ...
        $entityTypeId = $params['ENTITY_TYPE_NAME'] == "HMS" ? 38 : \CCrmOwnerType::ResolveID($params['ENTITY_TYPE_NAME']);
        if (
        ...
        if (!$this->isExistErrors()) {
            ...
            if ($this->entity['TYPE_NAME'] === \CCrmOwnerType::OrderName) {
                ...
                $this->entity['TYPE_CODE'] = $params['ENTITY_TYPE_NAME'] == "HMS" ? "HMS" : \CCrmOwnerTypeAbbr::ResolveByTypeName($this->entity['TYPE_NAME']);
            }
            // $this->entity['TYPE_ID'] = \CCrmOwnerType::ResolveID($this->entity['TYPE_NAME']);
            $this->entity['TYPE_ID'] = $entityTypeId;
    ...
    protected function initDefaultSettings(): void {
        ...
        $this->defaultSettings['NEW_ROW_POSITION'] = CUserOptions::GetOption('hms.entity.product.list', 'new.row.position', 'top');
        $this->defaultSettings['SHOW_PRODUCT_IMAGES'] = CUserOptions::GetOption('hms.entity.product.list', 'show.product.images', 'Y');
    ...
    protected function getMeasureByCode(string $code): ?array {
        ...
        return (!empty($this->measures['LIST'][$measureCode])
            ? $this->measures['LIST'][$measureCode]
            : null
        );
    }
    ...
    protected function checkModules(): bool {
        ...
        if (!Loader::includeModule('mywebstor.hms')) {
            $this->addErrorMessage('Module "mywebstor.hms" is not installed.');
    ...
    protected function loadData(): void {
        ...
        if (isset($this->arParams['~PRODUCTS']) && is_array($this->arParams['~PRODUCTS'])) {
            ...
            $this->rows = MyWebstorHmsHelper::fillProductRows($this->rows);
            if ($this->isAllowedReservation()) {
    ...
    protected function prepareResult(): void {
        ...
        $this->arResult['IS_INVENTORY_MANAGEMENT_TOOL_ENABLED'] = false;
    ...
    protected function getUiExtensions(): array {
        return [
            ...
            'mywebstor.hms.product-selector',
        ];
    }
    ...
    protected function getGridColumnsDescription(): array {
        ...
        $result['IS_VHI'] = [
            'id' => 'IS_VHI',
            'name' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_IS_VHI'),
            'sort' => '',
            'editable' => [
                'TYPE' => Types::CUSTOM
            ],
        ...
        if ($this->arParams["ENABLE_TEETH_FIELD"])
            $result['TEETH'] = [
                'id' => 'TEETH',
                'name' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_TEETH'),
                'sort' => '',
                'editable' => [
                    'TYPE' => Types::CUSTOM
                ],
        ...
        $result['IS_VHI']['default'] = true;
        if ($this->arParams["ENABLE_TEETH_FIELD"])
            $result['TEETH']['default'] = true;
    ...
    private function getRestrictedProductTypes(): array {
        return is_array($this->arParams["RESTRICTED_PRODUCT_TYPES"]) ? $this->arParams["RESTRICTED_PRODUCT_TYPES"] : [
