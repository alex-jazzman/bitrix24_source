 ...
 * @var \CUser $USER
 * @var \CDatabase $DB
 * @var \HmsDoctorListComponent $component
...
$addButton = new \Bitrix\UI\Buttons\AddButton(array(
    "click" => new \Bitrix\UI\Buttons\JsCode(
        'BX.SidePanel.Instance.open("/hms/config/doctor/details/0/");'
    )
));
...
<script>
    BX.ready(function() {
        var clinicList = BX.MyWebstor.HMS.Doctor.List.getInstance("<?= $arResult["GRID_ID"] ?>");
    });
</script>
