...
if ((!empty($_REQUEST["cancel"])) && check_bitrix_sessid())
{
	...
	?><?=CUtil::PhpToJSObject(array("result" => mb_strtolower($arParams["ACTION"]."ed"), "url" => $url));?><?
	die();
}
/********************************************************************
				/ CANCEL
...
if (isset($_REQUEST["ACTION"]) && (in_array($_REQUEST["ACTION"], array("LOCK", "UNLOCK"))))
{
	...
	if ($res["LOCK_STATUS"] != "green")
	{
		if ($res['WF_LOCKED_BY'] == $USER->GetID())
		{
			...
			$arUser = $rUser->Fetch();
			$res['LOCKED_USER_NAME'] = $arUser["NAME"].($arUser["NAME"] == '' || $arUser["LAST_NAME"] == ''?"":" ").$arUser["LAST_NAME"];
		}
...
elseif ($ob->workflow == "bizproc" && $_REQUEST["stop_bizproc"] <> '')
{
