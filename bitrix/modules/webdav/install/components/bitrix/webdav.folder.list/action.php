...
$_REQUEST["ACTION"] = mb_strtoupper($_REQUEST["ACTION"]);
/************** Folders and files **********************************/
...
elseif (!empty($_POST) && !empty($_REQUEST["FIELDS"]))
{
	...
	{
		if (mb_substr($key, 0, 1) == "S")
			$arData["S"][] = mb_substr($key, 1);
		else
			$arData["E"][] = mb_substr($key, 1);
	}
}
else
{
	...
	{
		$arData[(mb_substr($key, 0, 1) == "S"? "S" : "E")][] = mb_substr($key, 1);
	}
}
/************** Action *********************************************/
$ACTION = mb_strtoupper($_POST["action_button_".$arParams["GRID_ID"]]);
/************** Main errors ****************************************/
...
$arParams["CONVERT"] = (mb_strpos($arParams["~SECTIONS_URL"], "?") === false ? true : false);
if (!$arParams["CONVERT"])
	$arParams["CONVERT"] = (mb_strpos($arParams["~SECTIONS_URL"], "?") > mb_strpos($arParams["~SECTIONS_URL"], "#PATH#"));
