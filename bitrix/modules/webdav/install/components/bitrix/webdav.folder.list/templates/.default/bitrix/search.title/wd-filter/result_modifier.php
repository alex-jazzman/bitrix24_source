	...
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
        if (isset($arItem['ITEM_ID']))
        {
            if ($ob->Type == 'folder')
            {
				...
				$localpath = mb_substr($path, mb_strlen($ob->real_path));
                $arProps = $ob->_get_props($localpath);
                if (isset($arProps['UNDELETEBX:']))
            {
                    unset($arResult["CATEGORIES"][$category_id]["ITEMS"][$i]);
                    continue;
            }
            }
            elseif ($ob->Type == 'iblock')
            {

                $dbElement = $ob->_get_mixed_list(null, null, $arItem['ITEM_ID']);
                if ($dbElement && $arElement = $dbElement->Fetch())
                {
                    $ob->_parse_webdav_info($arElement);
                    if (isset($arElement['PROPS']['BX:']['UNDELETE']))
                    {
                        unset($arResult["CATEGORIES"][$category_id]["ITEMS"][$i]);
                        continue;
                    }
