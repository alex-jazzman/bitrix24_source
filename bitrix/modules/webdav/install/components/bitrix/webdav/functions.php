...
$file = trim(preg_replace("'[\\\\/]+'", "/", (__DIR__."/lang/".LANGUAGE_ID."/functions.php")));
__IncludeLang($file);
...
if (!function_exists("__parse_user"))
{
	function __parse_user($user_id, $user_url, $nameTemplate="")
	{
		...
		if (!empty($arUsersCache[$user_id]))
		{
			...
			{
				$newid = RandString(8);
				$pos = mb_strpos($arUser['main_user_link'], 'anchor_');
				while ($pos !== false)
				{
					$arUser['main_user_link'] = mb_substr($arUser['main_user_link'], 0, $pos + 7).$newid.mb_substr($arUser['main_user_link'], $pos + 15);
					$pos = mb_strpos($arUser['main_user_link'], 'anchor_', $pos + 14);
				}
			}
			return $arUser;
		}
...
if (!function_exists("__prepare_item_info"))
{
	function __prepare_item_info(&$res, $arParams = array())
	{
		if ($res["TYPE"] == "E")
		{
			...
			{
				if (mb_substr($key, -9, 9) != "_VALUE_ID" ||
					!(mb_substr($key, 0, 9) == "PROPERTY_" || mb_substr($key, 0, 10) == "~PROPERTY_"))
					continue;
				$key = mb_substr($key, 0, mb_strlen($key) - 9);
				$tmp[$key] = $res[$key."_VALUE"];
				if (mb_substr($key, 0, 9) == "PROPERTY_")
					$res["PROPERTIES"][mb_substr($key, 9)] = array("VALUE" => $res[$key."_VALUE"]);
			}
			$res["FILE_EXTENTION"] = htmlspecialcharsbx(mb_strtolower(strrchr($res['~NAME'], '.')));
		}
		...
		foreach ($res["URL"] as $key => $val)
		{
			...
			if (mb_substr($key, 0, 1) == "~" || array_key_exists("~".$key, $res["URL"]))
				continue;
			$res["URL"]["~".$key] = $val;
...
if (!function_exists("__make_hint"))
{
	function __make_hint(&$res, $prefix='')
	{
		...
		elseif ($res["PREVIEW_TEXT"] <> '')
			$text = trim($res["PREVIEW_TEXT"]);
		$text = HTMLToTxt($text);
		if (mb_strlen($text) < 1) return "";
		...
		if ($hint <> '') $hintLink = 'onmouseover="BX.hint(this, hint'.$prefix.$res['ID'].')"';
		else $hintLink = '';
		$res['HINT'] = ($hint <> '' ? "<script>var hint".$prefix.$res['ID']."='".CUtil::JSEscape($hint)."';</script>" : "");
		return $hintLink;
	}
}
...
/************** Grid Data ******************************************/
		...
		else
		{
			...
			if($allowExtDocServices && CWebDavTools::allowPreviewFile($res["FILE_EXTENTION"], $res['PROPERTY_WEBDAV_SIZE_VALUE']))
			{
				...
				$downloadUrl .= ((mb_strpos($downloadUrl, "?") === false) ? "?" : "&") . "ncc=1&force_download=1";
				...
				if (($res["SHOW"]["LOCK"] == "Y") || ($res["SHOW"]["UNLOCK"] == "Y"))
				{
					...
					if ($res["SHOW"]["BP_START"] == "Y" && is_array($arParams["TEMPLATES"]))
					{
						...
						foreach ($arParams["TEMPLATES"] as $key => $arWorkflowTemplate)
						{
							...
							$url .= (mb_strpos($url, "?") === false ? "?" : "&")."workflow_template_id=".$arWorkflowTemplate["ID"].'&'.bitrix_sessid_get();
							$arr[] = array(
								"ICONCLASS" => "",
		...
		else
		{
			...
			$aCols["NAME"]['icon'] = '<div class="element-icon icons icon-'.mb_substr($res["FILE_EXTENTION"], 1).'"></div>';
			$aCols["NAME"]['shared'] = ($res["SHOW"]["SHARED"])?'<div class="element-shared"></div>':'';
			...
			else
			{
				...
				if($ob->Type != "iblock")
				{
					$aCols["NAME"]['name'] = '<a class="element-title '.(($hintLink <> '') ? 'element-hint ' : ' ').
						'" id="doc'.$res['ID'].'" '.
						...
						&& ($arParams['DEFAULT_EDIT'] == 'Y')
							? "true" : "false").')"'.
						' target="_blank"'.($hintLink <> '' ? '' : ' title="'.GetMessage("WD_DOWNLOAD_ELEMENT").'"' ).
						'>' . $resName . '</a>';
				}
				else
				{
					$aCols["NAME"]['name'] =
						'<a class="element-title '.(($hintLink <> '') ? 'element-hint ' : ' ') .
						'" id="doc' . $res['ID'] . '" ' .
						$hintLink . ' ' .
						'data-bx-viewer="' . $arActions['preview_launch']['type'] . '" ' .
						'data-bx-title="' . htmlspecialcharsbx($arActions['preview_launch']['title']) . '" ' .
			...
			if ($arParams["WORKFLOW"] == "bizproc" && !empty($res["arDocumentStates"]))
			{
				...
				if (count($arDocumentStates) == 1)
				{
					...
					'<div class="bizproc-item-title">'.
						...
						'<span class="bizproc-item-title bizproc-state-title" style="">'.
							'<a href="'.$res["URL"]["BP"].'">'.
								($arDocumentState["STATE_TITLE"] <> '' ? htmlspecialcharsbx(htmlspecialcharsbx($arDocumentState["STATE_TITLE"])) : htmlspecialcharsbx(htmlspecialcharsbx($arDocumentState["STATE_NAME"]))).
							'</a>'.
						'</span>'.
					'</div>';
					$aColss["BIZPROC"] = str_replace("'","\"",$aColss["BIZPROC"]);
					$aCols["NAME"]['bizproc'] = "<div class=\"element-bizproc-status bizproc-statuses " . (!($arDocumentState["ID"] == '' || $arDocumentState["WORKFLOW_STATUS"] == '') ?
				...
				else
				{
					...
					foreach ($arDocumentStates as $key => $arDocumentState)
					{
						$arTasksWorkflow = CBPDocument::GetUserTasksForWorkflow($GLOBALS["USER"]->GetID(), $arDocumentState["ID"]);
						if (!$bInprogress)
							$bInprogress = ($arDocumentState["ID"] <> '' && $arDocumentState["WORKFLOW_STATUS"] <> '');
						$tmp[$key] =
							'<li class="bizproc-item">'.
								'<div class="bizproc-item-title">'.
									'<div class="bizproc-statuses '.
										($arDocumentState["ID"] <> '' && $arDocumentState["WORKFLOW_STATUS"] <> '' ?
