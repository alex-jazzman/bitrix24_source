...
/***************** URL *********************************************/
	...
	foreach ($URL_NAME_DEFAULT as $URL => $URL_VALUE)
	{
		...
		if (empty($arParams[mb_strtoupper($URL)."_URL"]))
			$arParams[mb_strtoupper($URL)."_URL"] = $GLOBALS["APPLICATION"]->GetCurPageParam($URL_VALUE, array("PAGE_NAME", "PATH",
...
********************************************************************/
	...
	if (!$bDialog)
	{
		...
		if (isset($_REQUEST["clear_filter"]) && $_REQUEST["clear_filter"] == "Y")
		{
			...
			if (mb_strpos($_SERVER['HTTP_USER_AGENT'], 'Firefox') === false)
		...
		foreach ($grid_filter as $key => $value)
		{
			...
			{
				$new_key = mb_substr($key, 0, -5)."_1";
			}
			elseif(mb_substr($key, -3) == "_to")
			{
				$new_key = mb_substr($key, 0, -3)."_2";
			}
			else
			{
				$new_key = $key;
		...
		foreach($dataNavResults as $res)
		{
			...
			if ($res["TYPE"] == "S")
			{
				...
				if (in_array("SECTION_CNT", $aColumns) || in_array("SECTIONS_CNT", $aColumns))
					$res["SECTION_CNT"] = $res["SECTIONS_CNT"] = intval(CIBlockSection::GetCount(array(
							"IBLOCK_ID"=>$arParams["IBLOCK_ID"],
							"SECTION_ID"=>$res["ID"])));
				if (in_array("ELEMENT_CNT", $aColumns) || in_array("ELEMENTS_CNT", $aColumns))
					$res["ELEMENT_CNT"] = $res["ELEMENTS_CNT"] = intval(CIBlockSection::GetSectionElementsCount(
...
/************** Grid Data ******************************************/
			...
			if ($res["TYPE"] == "E")
			{
				foreach ($res as $key => $val)
				{
					...
					{
						$tmp = mb_substr($key, 0, mb_strlen($key) - 6);
						$res[$tmp] = $val; 
					}
				}
