...
if(!function_exists("__CorrectFileName"))
{
	function __CorrectFileName(&$arFiles)
	{
		foreach ($arFiles as $key => $val):
			if (mb_strpos($key, "SourceFile_") === false || (mb_strpos($val["name"], "/") === false && mb_strpos($val["name"], "\\") === false))
				continue;
			$tmp = array();
			if (mb_strpos($val["name"], "/") !== false):
				$tmp = explode("/", $val["name"]);
			elseif (mb_strpos($val["name"], "\\") !== false):
				$tmp = explode("\\", $val["name"]);
			endif;
			if (!empty($tmp)):
...
/***************** BASE ********************************************/
	if (is_object($arParams["OBJECT"]))
		$arParams["RESOURCE_TYPE"] = mb_strtoupper($arParams["OBJECT"]->Type);
...
$arParams["NAME_FILE_PROPERTY"] = mb_strtoupper(trim(empty($arParams["NAME_FILE_PROPERTY"])? "FILE" : $arParams["NAME_FILE_PROPERTY"]));
...
/***************** URL *********************************************/
	...
	$arParams["CONVERT_PATH"] = (mb_strpos($arParams["~SECTIONS_URL"], "?") === false);
	if (!$arParams["CONVERT_PATH"])
		$arParams["CONVERT_PATH"] = (mb_strpos($arParams["~SECTIONS_URL"], "?") > mb_strpos($arParams["~SECTIONS_URL"], "#PATH#"));
	$arParams["CONVERT_PATH"] = (mb_strtolower($arParams["CONVERT"]) == "full" ? true : $arParams["CONVERT_PATH"]);
/***************** ADDITIONAL **************************************/
...
if ($arParams['ELEMENT_ID'] !== 0)
{
	if ($ob->Type == 'iblock')
	{
		...
		$res["FILE_EXTENTION"] = mb_strtolower(strrchr($res['NAME'], '.'));
	...
	elseif ($ob->Type == 'folder')
	{
		...
		{
			$arResult['ELEMENT']["FILE_EXTENTION"] = mb_strtolower(strrchr($arParams["ELEMENT_ID"], '.'));
		}
		else
		{
			__WDShowError(GetMessage("WD_ERROR_ELEMENT_NOT_FOUND"));
