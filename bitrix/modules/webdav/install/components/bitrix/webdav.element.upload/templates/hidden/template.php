...
if (isset($_REQUEST['CHECK_NAME']))
{
	...
	elseif (COption::GetOptionString('webdav', 'hide_system_files', 'Y') == 'Y' && mb_substr($fileName, 0, 1) == '.')
	{
		$result['permission'] = true;
		$result['okmsg'] = GetMessage("WD_WARNING_FIRST_DOT");
	}
	// don't check for unique file names
	elseif (($arParams['ELEMENT_ID'] !== 0) && ($arResult['ELEMENT']['FILE_EXTENTION'] != mb_strtolower(strrchr($fileName, "."))))
	{
		$result['permission'] = false;
