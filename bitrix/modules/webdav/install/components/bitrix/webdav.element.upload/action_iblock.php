...
class __WebdavUploader
{
	...
	function saveFile(&$name, $arFile, &$arElement)
	{
		...
		if($bDropped)
		{
			...
			{
				$countNonUnique++;
				$newName = mb_strstr($mainPartName, '.', true)." ({$countNonUnique})".mb_strstr($mainPartName, '.');
			}
			$name = $newName;
		}
		...
		if (($arParams['ELEMENT_ID']>0) && ($arResult['ELEMENT']['FILE_EXTENTION'] != mb_strtolower(strrchr($arFile["name"], '.'))))
		{
			$arFileError[] = array(
	...
	function getUrl()
	{
		...
		if ($arParams['ELEMENT_ID'] > 0) {
			$url .= $arResult["URL"]["VIEW"] . (mb_strpos($url, '?') === false ? '?':'&') . "result=uploaded";
		} else {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && empty($_POST))
{
	...
	if (mb_strpos(mb_strtolower($_SERVER['HTTP_USER_AGENT']), "opera") !== false)
