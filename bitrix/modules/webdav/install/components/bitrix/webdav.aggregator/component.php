...
$arResult["URL_TEMPLATES"]['user_view'] = CComponentEngine::MakePathFromTemplate($arParams["USER_VIEW_URL"], array("#USER_ID#" => $USER->GetID()));
	...
	if ($mode == 'private' || $arParams["EXPAND_ALL"] == "Y")
	{
		if (CSocNetFeatures::IsActiveFeature( SONET_ENTITY_USER, $currentUserID, "files")) 
		{
			...
			if ($ownerPerms >= "R")
			{
				...
				{
					$sectionID = $res['ID'];
					$arParams["ATTRIBUTES"] = array('user_id' => $USER->GetID());
			...
			if (sizeof($arLocalPath) > 0)
			{
				if (preg_match("/".GetMessage('SONET_GROUP')." (\d+)/", $arLocalPath[0], $matches) == 0)
				{
					...
					while ($arGroupSection = $dbSection->Fetch())
					{
						...
						if (mb_strpos($groupName, GetMessage('SONET_GROUP_PREFIX')) === 0)
						{
							$groupName = mb_substr($groupName, mb_strlen(GetMessage('SONET_GROUP_PREFIX')));
						}
