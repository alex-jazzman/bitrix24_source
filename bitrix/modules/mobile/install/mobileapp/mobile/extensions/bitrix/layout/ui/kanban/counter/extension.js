...
jn.define('layout/ui/kanban/counter', (require, exports, module) => {
	...
	class CounterComponent extends LayoutComponent
	{
				...
				View(
					...
					View(
						...
						Image({
							style: styles.icon,
							svg: {
								content: counters.length ? ICON : EMPTY_ICON,
							},
						}),
					),
				),
		...
		prepareBadges()
		{
			...
			const badges = counters.reverse().map((counter, index) => this.renderBadge(counter, totalCount, index, badgesCount));
			const flatten = arr => [].concat(...arr.map(n => Array.isArray(n) ? flatten(n) : n));

			return flatten(badges);
		}

		renderBadge(counter, totalCount, index, countersLength)
		{
			const commonBadgeStyles = styles.commonBadge;
			const badgeStyles = styles.badge(counter.color, countersLength, index);
		...
		renderIndicator()
		{
			const indicator = BX.prop.getObject(this.props, 'indicator', null);
			...
			return View({
				style: styles.indicator(indicator.color),
			});
		}
