...
CREATE TABLE `b_im_sharing_link`
(
	`ID` INT AUTO_INCREMENT,
	...
	`REQUIRE_APPROVAL` CHAR(1) NOT NULL DEFAULT 'N',
	`NAME` VARCHAR(255) DEFAULT NULL,

	PRIMARY KEY (`ID`),
	UNIQUE INDEX `UIX_B_IM_SHARING_LINK_1` (`CODE`),
	INDEX `IX_B_IM_SHARING_LINK_1` (`ENTITY_ID`, `ENTITY_TYPE`)
);
...
(
	...
	PRIMARY KEY (`ID`),
	KEY `IX_B_IM_STICKER_1` (`PACK_ID`)
);

CREATE TABLE `b_im_sticker_pack`
(
	`ID` INT NOT NULL AUTO_INCREMENT,
	...
	`DATE_CREATE` DATETIME NOT NULL,
	PRIMARY KEY (`ID`),
	KEY `IX_B_IM_STICKER_PACK_1` (`AUTHOR_ID`)
);

CREATE TABLE `b_im_sticker_user_pack`
(
	`ID` INT NOT NULL AUTO_INCREMENT,
	`PACK_ID` INT NOT NULL,
	`USER_ID` INT NOT NULL,
	`DATE_CREATE` DATETIME NOT NULL,
	PRIMARY KEY (`ID`),
	UNIQUE KEY `UIX_B_IM_STICKER_USER_PACK_1` (`USER_ID`, `PACK_ID`),
	KEY `IX_B_IM_STICKER_USER_PACK_1` (`PACK_ID`)
);
